...
`PUT /v2/partners` - update profile info
...
`GET /v2/partners/coupons` - get list of active coupons
...
`GET /v2/partners/earnings` - get earnings (commission earned, balance, blocked, pending amount and more)
...
`POST /v2/partners/withdrawals` - create withdrawal request
...
`GET /v2/partners/withdrawals` - get list of withdrawals
...
- `date_from` - get report for withdrawals from a given date
- `date_to` - get report for withdrawals to a given date
...
- `label` - **required** - custom label for the token (min. 3 characters)
...
- `link` - URL of product landing page (default: product homepage)
- `trial_duration` - only for `livechat` product - days of trial period (mix. 14 - default, max. 60)
- `coupon_id` - only for `livechat` product - ID of the coupon assigned to partner (will apply a discount)
...
`DELETE /v2/partners/affiliates/campaigns` - disable campaign
...
`GET /v2/partners/affiliates/licenses` - get list of Affiliate licenses
...
- `date_to` - get licenses created to a given date
...
- `date_from` - get report for licenses created from a given date
- `date_to` - get report for licenses created to a given date
...
`GET /v2/partners/affiliates/partners-referred` - get list of referred Partners
...
- `purchase_order` - custom parameter
- `trial_duration` - only for `client` payment origin - days of trial period (mix. 14 - default, max. 60)
- `coupon_id` - only for `client` payment origin - ID of the coupon assigned to partner (will apply a discount)
...
`GET /v2/partners/solutions/licenses` - get license list
...
`GET /v2/partners/solutions/licenses/<license_id>` - get license details
...
`GET /v2/partners/solutions/licenses/<license_id>/operators` - get list of operators
...
`GET /v2/partners/solutions/licenses/<license_id>/invoices` - get invoices for given license
...
`GET /v2/partners/solutions/licenses/<license_id>/invoices/<invoice_no>` - get PDF invoice
...
`GET /v2/partners/solutions/history` - get historical stats for Solution Partner activity (up to last 12 months)
...
`POST /v2/partners/solutions/licenses/<license_id>/subscription` - create new subscription
...
`PUT /v2/partners/solutions/licenses/<license_id>/subscription` - update subscription
...
`DELETE /v2/partners/solutions/licenses/<license_id>/subscription` - close subscription
...
- `expire` - `boolean` value (default: `false`). When closing subscription your license will expire at the end of its billing cycle. To expire your subscription immediately, set `expire` param to `true`.
...
`PUT /v2/partners/solutions/licenses/<license_id>/billing` - update subscription billing information
...
`POST /v2/partners/solutions/licenses/<license_id>/transfer` - transfer license to the client
...
- `new_owner` - **required** - new owner email (one of [operators](#get-license-operators))
- `password` - **required** - your password in Partner Program
