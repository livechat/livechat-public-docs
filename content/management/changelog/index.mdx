...
- The [**Chat access webhooks**](/management/configuration-api/v3.2/#chat-access) `access_set`, `access_revoked` and `access_granted` were renamed to `chat_access_set`, `chat_access_revoked` and `chat_access_granted`. These webhooks no longer include `resource` parameter.
