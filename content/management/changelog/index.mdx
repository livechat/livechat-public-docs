...
- The [**Unregister Webhook**](/management/configuration-api/v3.3/#unregister-webhook) method:
  ...
  - had required scope changed to `webhooks.configuration:rw`.
  - has a new parameter `owner_client_id`. It's required when authorizing with Personal Access Token; otherwise ignored.
...
- The [**Register Webhook**](/management/configuration-api/v3.3/#register-webhook) method:
  - had the response parameter `webhook_id` renamed to `id`
  - had required scope changed to `webhooks.configuration:rw`.
  - has a new parameter `owner_client_id`. It's required when authorizing with Personal Access Token; otherwise ignored.
