...
This document describes the **Configuration API v3.4**. This is the developer preview version recommended for experimenting with the API capabilities. For production-ready applications, use the currently stable version. [Read more about versioning...](/messaging/#api-versioning)
...
To receive the [`license`](#request-29) type webhooks, you need to enable them after registration. You can do that by calling the [Enable License Webhooks](#enable-license-webhooks) method that will install your registered webhooks for a given license. Keep in mind this two-step process applies to **license webhooks**.

For the [`bot`](#request-29) type webhooks, you need to [create a bot](#create-bot), [register webhooks](#register-webhook), and then, call [Set Routing Status](/messaging/agent-chat-api/v3.4/#set-routing-status).

As an alternative to calling Register Webhook, you can register webhooks via {{DEVELOPER_CONSOLE_URL}} by configuring the Chat Webhooks building block. In such a case, you won't have to support enabling webhooks as LiveChat will do that under the hood when someone installs your app. [Learn more...](/getting-started/app-guides/chat-webhooks/)
