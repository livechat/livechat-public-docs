...
Creates a new Bot. If the bot should be available for getting chats or receiving webhooks, use [`set_routing_status`](/messaging/agent-chat-api/v3.4/#set-routing-status) method.
...
| `owner_client_id` __*****__          | Yes      | `string`   | Required when authorizing via [PATs](/authorization/authorizing-api-calls/#personal-access-tokens); rejected otherwise. |
...
Registers a webhook for the Client ID (application) provided in the request. To activate the license webhooks, you can [enable](#enable-license-webhooks) registered webhooks for a given license. To activate the bot webhooks, you need to [create a bot](#create-bot) and then use [`set_routing_status`](/messaging/agent-chat-api/#set-routing-status) method. One Client ID can register up to three webhooks for every `action`.
...
| Parameter         | Required | Data type | Notes                                                                                                                  |
| ----------------- | -------- | --------- | ---------------------------------------------------------------------------------------------------------------------- |
| `owner_client_id` | Yes      | `string`  | Required when authorizing via [PATs](/authorization/authorizing-api-calls/#personal-access-tokens); rejected otherwise. |
