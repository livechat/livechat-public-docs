...
This document describes the **Configuration API v3.2**. This is the **developer preview** version recommended only for the **experimental** use. [Read more about versionig...](/messaging/#api-versioning)
...
Creates new agent on license for specified parameters. If requester has scope `billing:rw`, then newly created agent will have property `awaiting_approval` marked as `false`, otherwise it will be `true`.
...
| `role`                                   | `string`   | No       | Agent role, should be on of following: `viceowner`, `administrator`, `normal` |
...
| `design_version`                         | `bool`     | No       | UI version enabled for agent (???)                                            |
| `chats_enabled`                          | `bool`     | No       | Is it realiable??                                                             |
| `customers_enabled`                      | `bool`     | No       | Is it realiable??                                                             |
| `customers_visited`                      | `bool`     | No       | Is it realiable??                                                             |
...
| `email_subscriptions`__****__            | `string[]` | No       | Represents which subscriptions will be send to agent via email.               |
| `work_scheduler`                         | `object`   | No       | Work scheduler options to set for new agent                                   |
| `work_scheduler.__**__.<work_scheduler>` | `object`   | No       | **\**\*** **described below**                                                 |

**\**\***
`<work_scheduler>` contains the following fields:

- `start` (`string`) - time when agent starts work, in format: `HH:MM`
- `end` (`string`) - time when agent finishes work, in format: `HH:MM`
...
__***__ Possible values for the `notifications` array (value present in array means that notification is turned on):
- `new_visitor`
- `incoming_chat`
- `returning_visitor`
- `queued_visitor`
...
__****__ Possible values for the `email_subscriptions` array (value present in array means that notification is turned on):
- `daily_summary`
- `weekly_summary`
...
| --------------------- | ---------- | -------- | ----------------------------------------------------------------------- |
| `id`                  | `string`   | Yes      | Agent ID                                                                |
| `with_work_scheduler` | bool       | No       | Indicate if return `work_scheduler` object for given agent              |
| `with_groups`         | bool       | No       | Indicate if return `groups` object for given agent                      |
| `fields`__*__         | `string[]` | No       | Filter out desired Agent fields. When empty, you'll get all the fields. |
...
|------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `email`                | Agent's email                                                                                                                                                                                                                        |
| `notifications`        | Agent's notifications object                                                                                                                                                                                                         |
| `summaries`            | Agent's summaries object                                                                                                                                                                                                             |
| `job`                  | Agent's job title                                                                                                                                                                                                                    |
| `login_status`         | The current login status of an Agent                                                                                                                                                                                                 |
| `onboarding_completed` | Indicates if an Agent finished the onboarding process, which includes: activating the account, downloading the LiveChat Desktop App, uploading an avatar, having at least one chat. `1` when all steps are completed, otherwise `0`. |
| `awaiting_approval`    | Boolean value                                                                                                                                                                                                                        |
| `permission`           | Agent permission, should be on of following: `viceowner`, `administrator`, `normal`                                                                                                                                                  |
| `avatar`               | URL path for agent avatar                                                                                                                                                                                                            |
| `mobile`               | Agent's mobile number                                                                                                                                                                                                                |
| `max_chats_count`      | Agent's maximum number of concurrent chats                                                                                                                                                                                           |
| `design_version`       | UI version enabled for agent (???)                                                                                                                                                                                                   |
| `chats_enabled`        | Is it realiable??                                                                                                                                                                                                                    |
| `customers_enabled`    | Is it realiable??                                                                                                                                                                                                                    |
| `customers_visited`    | Is it realiable??                                                                                                                                                                                                                    |
...
Returns all Agents within a license. You can request only the Agents from a specific group as well as filter Agent fields.
...
|---------------|----------|------------|-------------------------------------------------------------|
| `group_ids`   | No       | `int[]`    | Groups from which Agents you want to receive.               |
| `fields`__*__ | No       | `string[]` | Agent fields to filter out. At least one field is required. |
...
| `job`                  | Agent's job title                                                                                                                                                                   |
| `login_status`         | The current login status of an Agent                                                                                                                                                |
| `work_scheduler`       | Work scheduler object for agent                                                                                                                                                     |
| `groups`               | Groups in which agent belongs                                                                                                                                                       |
| `email_subscriptions`  | Active agent email subscriptions list                                                                                                                                               |
| `notifications`        | Enabled agent email subscriptions list                                                                                                                                              |
| `onboarding_completed` | Indicates if an Agent finished the onboarding process, which includes: activating the account, downloading the LiveChat Desktop App, uploading an avatar, having at least one chat. |
| `awaiting_approval`    | Boolean value                                                                                                                                                                       |
| `permission`           | Agent permission, should be on of following: `viceowner`, `administrator`, `normal`                                                                                                 |
| `avatar`               | URL path for agent avatar                                                                                                                                                           |
| `mobile`               | Agent's mobile number                                                                                                                                                               |
...
| `groups[].id`                            | `uint`     | Yes      | Group ID; required only when `group`'s included.                                         |
| `groups[].priority` __****__             | `string`   | Yes      | Agent's priority in a group; required only when `group`'s included.                      |
| `notifications`                          | `string[]` | No       | Represents which agent notifications are turned on, possible values: __***__             |
| `email_subscriptions`                    | `string[]` | No       | Represents which subscriptions will be send to agent via email, possible values: __***__ |
| `work_scheduler`                         | `object`   | No       | Work scheduler options to set for new agent                                              |
| `work_scheduler.__**__.<work_scheduler>` | `object`   | No       | **\**\*** **described below**                                                            |
...
Suspends Agent specified by `id`.
...
Unsuspends Agent specified by `id`.
...
Suspended agent can send email to all owners and viceowners with request to unsuspend him.
...
Approves agent making him able to use application.
...
| **Push equivalent in** | [`Agent Chat API`](/messaging/agent-chat-api/v3.2/rtm-reference/#chat_access_set) [`Customer Chat API`](/messaging/customer-chat-api/v3.2/rtm-reference/#access_set)   |
