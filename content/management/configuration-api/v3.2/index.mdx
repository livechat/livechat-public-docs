...
It returns the info about an Agent specified by `id`. There are some agent fields that always gonna be returned, but user can request some additional ones
...
| `custom_fields`__**__ | `string[]` | No       | Additional custom fields to include. |
...
__**__ The table below presents the possible values for the `custom_fields[]` parameter:
