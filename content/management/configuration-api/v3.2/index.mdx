...
Sets a property value within a license as the [property location](#property-locations). This operation **doesn't** overwrite the existing values.
...
Otherwise, the permission depends on how the property was configured when [creating](#create-properties) (`<property_name>.locations.<location>.access.<user>.write`).
...
__*)__ If the requester's Client ID matches the namespace, setting license properties is always allowed. 
Otherwise, the permission depends on how the property was configured when [creating](#create-properties) (`<property_name>.locations.<location>.access.<user>.read`).
...
| Parameter          | Required | Data type               | Notes                                                                               |
| ------------------ | -------- | ----------------------- | ----------------------------------------------------------------------------------- |
| `group_id`         |   Yes    | `number`                | Group ID to set the properties for.                                                 | 
| `properties`       |   Yes    | [`object`](#properties) | An object with _namespaces_ as keys and _properties_ (grouped in objects) as values.| 
...
| ---------------------------------------- | ---------- | -------- | ---------------------------------------------------------------------------------------- |
| `action`__*__                            | Yes  |  `string`     | The [triggerring action](#triggering-actions).                                                                    |
...
| Possible value        | Triggered by push        | Available filters  |
| ------------- |:-------------:| -----:|
| `incoming_chat_thread` | [`incoming_chat_thread`](/../../messaging/agent-chat-api/rtm-reference/#incoming-chat-thread) | `chat_member_ids`, `only_my_chats` |
| `incoming_event`  | [`incoming_event`](/../../messaging/agent-chat-api/rtm-reference/#incoming-chat-thread)  |   `chat_member_ids` and `author_type`, `only_my_chats` |
| `incoming_rich_message_postback` | [`incoming_rich_message_postback`](/../../messaging/agent-chat-api/rtm-reference/#incoming-rich-message-postback) | `chat_member_ids`, `only_my_chats`   |
...
| `agent_status_changed`  | [`agent_status_changed`](/../../messaging/agent-chat-api/rtm-reference/#agent_status_changed)   | `chat_member_ids`   |
| `agent_deleted`  | [`agent_deleted`](/../../messaging/agent-chat-api/rtm-reference/#agent_deleted)   |  `chat_member_ids`  |
