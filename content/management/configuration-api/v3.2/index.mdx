...
Creates new agent on license for specified parameters.
...
| `groups.<group>`                         | `array`   | No       | **\*\*** **described below**                                            |
| `notifications`                          | `object`  | No       | Represents which agent notifications are turned on |
| `notifications`.__***__                  | `bool`    | No       | Represents if given notifications is turned on |
| `summaries`                              | `object`  | No       | Represents which summaries will be send to agent via email |
| `summaries`.__****__                     | `bool`    | No       | Represents if given summary is active for agent |
...
| `__*__`                                  | `any`     | No       | Value for given agent field.                                            |

**\*\***
`<group>` contains the following fields:

- `id` (`int`) - must be greater or equal to 0
- `priority` (`string`) - allowed values: `first`, `normal`, `last`

**\**\***
`<work_scheduler>` contains the following fields:

- `enabled` (`boolean`) - indicates if works scheduler for given day is enabled
- `start` (`string`) - time when agent starts work, in format: `HH:MM`
- `end` (`string`) - time when agent finishes work, in format: `HH:MM`

__*__ The table below presents the possible values for the `fields` object keys:
...
__***__ Possible values for the `notifications` object keys:
- `new_visitor`
- `incoming_chat`
- `returning_visitor`
- `queued_visitor`
...
__***__ Possible values for the `summaries` object keys:
- `daily_summary`
- `weekly_summary`
...
| `with_notifications`  | bool       | No       | Indicate if return `notyfications` object for given agent               |
| `with_summaries`      | bool       | No       | Indicate if return `summaries` object for given agent                   |
...
| `group_ids`           | No       | `int[]`    | Groups from which Agents you want to receive. If `group_ids` are provided, then also `fields` is mandatory and cannot exceed 5 elements. |
| `with_work_scheduler` | No       | bool       | Indicate if return `work_scheduler` object for given agent                                                                               |
| `with_groups`         | No       | bool       | Indicate if return `groups` object for given agent                                                                                       |
| `with_notifications`  | No       | bool       | Indicate if return `notifications` object for given agent                                                                                |
| `with_summaries`      | No       | bool       | Indicate if return `summaries` object for given agent                                                                                    |
| `fields`__*__         | No       | `string[]` | Filter out desired Agent fields. When empty, you'll get all the fields.                                                                  |
