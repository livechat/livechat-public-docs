...
Creates new agent on license for specified parameters. If requester has scope `billing:rw`, then newly created agent will have property `awaiting_approval` by default marked as `false`, otherwise it will be `true`.
...
| `role`                                   | `string`   | No       | Agent role, should be on of following: `viceowner`, `administrator`, `normal`. Default role is `normal`.                               |
...
| `awaiting_approval`                      | `bool`     | No       | Determines if agent will be awaiting for approval after creation. Only requester with scope `billing:rw` can set this value to `false` |
...
| `suspended`           | Boolean value  indicating if agent is suspended             |
| `awaiting_approval`   | Boolean value  indicating if agent is awaiting for approval |
...
Returns all Agents within a license. You can request only the Agents from a specific group. Some of agents fields are returned by default by you can also extend them via providing fields names.
...
|---------------------|----------|------------|-----------------------------------------------|
| `filters`           | No       | `object`   | Groups from which Agents you want to receive. |
| `filters.group_ids` | No       | `int[]`    | Groups from which Agents you want to receive. |
...
[
    {
        "id": "agent@domain.com",
        "name": "John",
        "avatar_path": "https://domain.com/avatar_image.jpg",
        "role": "administrator",
        "login_status": "accepting chats",
        "job_title": "Support Hero",
        "max_chats_count": 5,
    ...
    },
    {
        ...
        "name": "Adam",
        "avatar_path": "https://domain.com/avatar_image1.jpg",
        "role": "normal",
