...
Informs that a chatting Customer's session fields were updated.
The webhook will be sent only if customer has active chats. If a customer is in more than one chat, we send as many webhooks as there are chats for given customer.
