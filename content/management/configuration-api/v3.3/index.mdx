...
You can register, unregister, and publish properties for **Client Ids** owned by your organization, and configure them using the **Configuration API**. Properties are grouped in _namespaces_, which helps distinguishing which property belongs to a given integration. The _namespace_ of the properties registered for **Client Ids** owned by your organization is always named after that **Client Id**.

You can configure the property [type](#property-types), [location access](#property-locations), and [domain](#property-domain).

You can [publish the property](#public-properties) so that it'll be available to other integrations.

There are also [predefined public properties](/messaging/references/default-properties/#available-properties) used mainly by the LiveChat backend applications.
...
Unpublished (private) properties can only be accessed using tokens belonging to the **Client Id** that owns those properties. If the property should be accessible using any token, it must be [published](#publish-property). Published properties cannot be unregistered.

You can configure access to public property for tokens NOT belonging to the **Client Id** that owns those properties. You can set it to `read` or `write`.
For example, if you create a chat property with `read` and `write` access, you can narrow it's public access to `read`. In this case tokens belonging to the **Client Id** that owns those properties will be able to read and write to that property, but any other token will only be able to read from it.
