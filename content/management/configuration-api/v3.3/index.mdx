...
Lists all the Bots within a license that were created by the requester. The method behavior differs depending on who created the Bot and who the requester is.

- If a Bot was created **directly by an Agent** (PAT authorization), all Agents within the license will get that Bot in the response, even with the `all` parameter set to `false`.
- In a situation when a Bot was created by an Agent **via an application**, all Agents within a license will get the Bot in the response if they request it via that application. If the Agents, including the Bot creator, want to access it via a different application, they need to specify `all:true`.
...
| Parameter | Required | Type   | Notes                                                                         |
| --------- | -------- | ------ | ----------------------------------------------------------------------------- |
| `all`     | No       | `bool` | Get all Bots, if `false` returns only requester's Bots, default value is `false` |
...
<CodeResponse type="configuration" version="v3.3" json="listRegisteredProperties"/>
