...
| `normal`       | The mediumn chat routing priority. Agents with the `normal` priority get chats before those with the `last` priority, when there are no Agents with the `first` priority available with free slots in the group. |
...
| `last`          | The lowesst chat routing priority. Agents with the `last` priority get chats when there are no Agents with the `first` or `normal` priority available with free slots in the group.   |
...
| `event_updated`                   | [`event_updated`](/messaging/agent-chat-api/rtm-reference/#event_updated)                                   | `chat_member_ids`, `author_type`, `only_my_chats` |
