...
Note: When your server receives a webhook from LiveChat, it should respond with **HTTP 200** response. Otherwise, LiveChat will retry sending the webhook to your service for a number of times unless it receives the correct **HTTP 200** response.

The timeout is set to ~10 seconds and if in that time we won't receive **HTTP 200** response we'll retry sending the webhook up to 10 times within 6 hours.
