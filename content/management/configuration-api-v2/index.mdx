...
<Code>
<CodeSample path={'POST /chats/<CHAT_ID>/send_transcript'}>
<Sample label={'cURL (JSON)'}>
...
If your endpoint is responsible for both receiving data from our webhook and making further actions, like creating record in DB or any other actions, then the whole process may take longer than 10 seconds.
In such case we believe that the best solution would be to split your mechanism into 2 separate actions:

1) retrieving data from LiveChat webhook (sending http code 200 back to us immediately)

2) further processing this data and passing it to other services

That would prevent situations in which the process takes longer than our webhook timeout and data would not be pushed again from our side.
