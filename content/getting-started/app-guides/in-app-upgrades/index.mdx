...
2. Then, the customer purchased it again with `quantity: 2`.
3. To disable the upgrade, two actions need to be performed: a disable action with `quantity: 1` and another disable action with `quantity: 2` (the order of actions doesn't matter). The important thing is, it cannot be a single action with `quantity: 3`.
