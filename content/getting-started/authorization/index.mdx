...
Every product defines it's own roles. There are three role types:
- `primary` - it's a required role in an organization (Accounts product), always set
- `secondary` - required for a given product, when the product is not enabled, the role can be not set
- `custom` - it's an optional role with additional permissions
