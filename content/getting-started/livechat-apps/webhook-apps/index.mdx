...
# Basic tutorial (webhooks v2)

Please note that this configuration uses [Configuration API v2 webhooks](/management/configuration-api/v2.0/#create-a-new-webhook), which are now deprecated. We highly recommend using the [Configuration API v3 webhooks](/management/configuration-api/#webhooks).
...
### Seting up webhooks v2 for development

One of the possible development options is to set up your webhooks from within the LiveChat Agent App. To do so, go to **Settings > Integrations > [Webhooks](https://my.livechatinc.com/settings/webhooks)** and set up your webhook URLs.
