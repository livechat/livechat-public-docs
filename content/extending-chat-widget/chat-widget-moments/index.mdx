...
<div>
  <iframe
    src="https://cdn.livechatinc.com/labs/open_chat3.html?license_id=100016461&group=1"
...
Moments can be triggered only by [rich message's](https://www.livechat.com/help/rich-messages/) buttons. Currently, LiveChat supports rich messages delivered by chatbots using our [ChatBot](https://chatbot.com) product, a [MessageBox](/extending-ui/extending-agent-app/agent-app-sdk/#messagebox) integration in Agent App, or by sending a rich message using our [Agent Chat API](/content/messaging/agent-chat-api/). We are still working on other possibilities to provide rich messages to chat, such as using backend Chat APIs.
