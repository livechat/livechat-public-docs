...
The app uses the **LiveChat OAuth token** to authorize requests sent to your backend service. To confirm that the user is authorized, [validate a given token](/getting-started/authorization/#validating-the-access-token) and check if it matches the Product Cards client ID **and** your license number.
