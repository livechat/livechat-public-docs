...
Dismisses currently displayed greeting.
In order to dismiss certein greeting you can listen to [`new_event`](#on-new-event) or [`greeting_dismissed`](#on-greeting-dismissed) callbacks to know when greeting with specified `id` or `uniqueId` appears and disappears.
...
Called after the greeting has been dismissed y customer or JS API
...
function onGreetingDismissed(greeting) {
  ...
  var id = greeting.id
  var uniqieId = greeting.uniqieId
}
...
Called after the rich message button has been clicked by customer
...
| **If button's owner event is a greeting:** |          |                              |
| greeting.id                                | `number` | Greeting's template id       |
| greeting.uniqueId                          | `string` | Greeting's event uniqueId    |
...
function onRichMessageButtonClicked(data) {
  ...
  if (data.greeting) {
    ...
    var greetingUniqieId = data.greeting.uniqieId
  }
}
