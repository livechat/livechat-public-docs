...
This document focuses on Developers and requires a basic knowledge of JavaScript. However, if you would have any questions, don't hesitate to start a chat with our Support Team or add a new topic on [Developers Community](https://www.livechatinc.com/community/c/developers-api).
...
- [Hide](#hide)
- [Destroy](#destroy)
- [Dismiss greeting](#dismiss-greeting)
...
It dismisses the currently displayed greeting.
In order to dismiss particular greeting you can listen to [`new_event`](#on-new-event) or [`greeting_dismissed`](#on-greeting-dismissed) callbacks to know when greeting with specified `id` or `uniqueId` appears and disappears accordingly.

#### Example

```js
LiveChatWidget.call('dismiss_greeting')
```
...
It is called after the greeting has been dismissed by the customer or [`dismiss_greeting`](#dismiss-greeting) method.
...
```js
function onGreetingDismissed(greeting) {
  // greeting has been dissmised
  var id = greeting.id
  var uniqueId = greeting.uniqueId
}

LiveChatWidget.on('greeting_dismissed', onGreetingDismissed)
LiveChatWidget.off('greeting_dismissed', onGreetingDismissed)
```
...
LiveChatWidget.on('rich_greeting_button_clicked', onRichMessageButtonClicked)
LiveChatWidget.off('rich_greeting_button_clicked', onRichMessageButtonClicked)
```
