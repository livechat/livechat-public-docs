...
This method facilitates loading more history threads events. You need to
...
This method doesn't return a promise. It sends the request if the SDK is connected or queues the update to be performed once the connection gets established.
...
This method is a little bit special - it returns both a `promise` and a `cancel` which you can use to abort the upload of
the file. It also lets you pass `onProgress` callback function. Keep in mind that the maximum accepted file size is 10 MB.
