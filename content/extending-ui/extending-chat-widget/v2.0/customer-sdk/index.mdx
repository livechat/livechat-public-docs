...
e.g. JavaScript)_ type). Then, you will have to add it the access to the `customers:own` scope, and the correct URI to the `Redirect URI whitelist`.
...
If you are looking for something simpler, you can use a <a href="https://github.com/livechat/react-native-livechat" target="_blank">LiveChat for React Native</a> library. This is a React Native component to easily add LiveChat widget with basic functionality to your application.
...
- Updated `@livechat/customer-auth` dependency which has been previously published with broken dist files.
