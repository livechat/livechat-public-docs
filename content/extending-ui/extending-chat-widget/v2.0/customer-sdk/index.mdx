...
`<script src="https://unpkg.com/@livechat/customer-sdk@2.0.0-beta.2"></script>`
...
Returns the chat data about the requested chat ID together with a single thread's data. If the method is called with `threadId` parameter, then this particular thread is being returned. If no `threadId` is given the latest thread is automatically returned.
...
- `value` - an object with array of threads, each containing an array of its events
...
| parameters | type   | description                                                                 |
| ---------- | ------ | --------------------------------------------------------------------------- |
| pageId     | string | Optional; Cursor returned from previous `listChats` calls                   |
| limit      | number | Optional; Limit of returned results. Default is set to 10 and maximum is 25 |
...
Returns a list of thread objects together with previous and next page ID cursors that can be used to load more threads.
...
| -------------- | --------------- | ------------------------------------------------------------------ |
| chatId         | string          |                                                                    |
| pageId         | string          | Optional, cursor returned from previous `listThreads` calls        |
...
| threads[].queue.position | number   | Current place in the queue                                                 |
...
## [v2.0.0-beta.3] - xxxx-xx-xx

### Changed

- The iterator returned from `getChatHistory` no longer returns `users`. Their data needs to be obtained independently, for example by using `getChat` method.
- `listChats` no longer accepts `offset` parameter. Instead, it now accepts `pageId` to which you can provide a value of `previousPageId` or `nextPageId` returned from other `listChats` calls.
- Chat objects returned from `listChats` no longer have `lastThreadOrder` property, but they have received a new `lastThreadCreatedAt` property instead.
- `threads` objects no longer have `order` property, but they have received a new `createdAt` property instead.
- `eventsSeenUpToMap` is no longer available on thread object, it has been moved to chat objects.
- The payload of the `chat_transferred` event has been changed. It now always have a `reason` property, contains `queue` property if the chat has been transferred and queued immediately and `requesterId` is available when `reason` is equal to `"manual"`.

### Removed

- `access_set` event has been removed. `chat_transferred` event covers for its use cases.
- `getChatThreads` and `getChatThreadsSummary` methods have been removed. Newly introduced `listThreads` can be used instead.

### Added

- `listThreads` and `getChat` methods have been added.
- Thread objects gained back `userIds` property.
- Thread objects got `previousThreadId` and `nextThreadId` properties.
- `sendTicketForm` methods accepts a new `timeZone` parameter which influences how times are formatted in a generated ticket.
