...
Keep in mind that interacting with this API requires **some development skills**. Additionally, it allows you to create frontend apps.
...
We provide an asynchronous API. Most methods return a [`Promise`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). You can subscribe to emitted events with the `on` and `off` methods.
...
and expose the created `auth` object to the returned SDK's instance. In general,
...
We have prepared a sample chat widget implementation to present the features of LiveChat Customer SDK: [Sample widget on CodeSandbox](https://codesandbox.io/s/rm3prxw88n)

## Known issues

Currently, LiveChat Customer SDK doesn't support queue handling. However, it will be added soon. If you have any questions, you can start a chat with our [24/7 Support](https://direct.lc.chat/1520/125).
...
as a "Web app (frontend, eg. JavaScript)" type. Then, you have to pass the configured <code>redirectUri</code> to the <code>init</code>, along with the regular required properties (<code>licenseId</code> and <code>clientId</code>).</div>

## Use the API

Now run the `init` function with the configuration, replacing `LICENSE_ID`
with your LiveChat license ID.

It also accepts some optional parameters:
...
The LiveChat system includes four basic types of entities - users, chats, threads, and
events.
...
The Customer SDK is providing wrappers for common tasks and most methods expect to receive chat IDs.
...
| parameters | type   | description                                             |
| ---------- | ------ | ------------------------------------------------------- |
| chatId     | string | id of the chat of which properties you want to delete   |
| threadId   | string | id of the thread of which properties you want to delete |
...
| chatId     | string | id of the chat of which properties you want to delete   |
| threadId   | string | id of the thread of which properties you want to delete |
| eventId    | string | id of the event of which properties you want to delete  |
| properties | object | properties to be deleted                                |
...
This is useful for implementing an infinite scroll or otherwise showing your customer archival chats.
...
| parameters | type   | description                         |
| ---------- | ------ | ----------------------------------- |
| chatId     | string | chat id of requested history object |
...
|              |          | lastSeenTimestamps | object   | map from user IDs to optional lastSeenTimestamps   |
|              |          | lastThread         | string   | thread ID                                          |
| totalChats   | number   |                    |          |                                                    |

## getChatThreads

Returns a list of thread objects for the specified thread IDs.
...
Returns a summary of your chat threads, including the total count of threads, a list of IDs, and a number of events in each thread.
If you want to load more thread summaries, consider using [`getChatHistory`](#getchathistory).
...
| parameters | type     | description                                              |
| ---------- | -------- | -------------------------------------------------------- |
| groupIds   | number[] | optional, if omitted statuses of all groups are returned |
...
```js
customerSDK
  .getUrlDetails({ url: 'https://www.livechatinc.com' })
  .then(urlDetails => {
    if (urlDetails.title) {
...
Sends a provided object as an event with specified type.
...
Sends infomration to the server about a user interaction with a rich message button.
...
Sends the request if the SDK is connected. Alternatively, queues the update to perform once the connection gets established.
...
It is sent to the server only if the target chat is active.
...
- `GROUPS_OFFLINE` - a group in the targeted chat is offline. It can happen for licenses, which don't allow customers to chat during their unavailability.
...
| parameters | type   | description                                           |
| ---------- | ------ | ----------------------------------------------------- |
| chatId     | string | id of the chat of which properties you want to update |
| properties | object |
...
| ---------- | ------ | ------------------------------------------------------- |
| chatId     | string | id of the chat of which properties you want to update   |
| threadId   | string | id of the thread of which properties you want to update |
| properties | object |
...
| chatId     | string | id of the chat of which properties you want to update   |
| threadId   | string | id of the thread of which properties you want to update |
| eventId    | string | id of the event of which properties you want to update  |
| properties | object |                                                         |
