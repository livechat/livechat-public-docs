...
LiveChat Customer SDK is a set of tools utilizing the [LiveChat Customer Chat API](https://developers.livechatinc.com/docs/messaging/customer-chat-api/) that help you build a custom chat widget. It allows you to communicate with LiveChat services directly from the browser environment using JavaScript, without the need to develop your backend.
...
Keep in mind that interacting with this API requires **some development skills**. Additionally, it allows you to create front-end apps, so if you are searching for the server-side solutions, you should explore the [LiveChat Customer Chat API](https://developers.livechatinc.com/docs/messaging/customer-chat-api/) instead.
...
We provide an asynchronous API. Most methods return [`Promise`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). To the emitted events you can subscribe with `on` and `off` methods.
...
The LiveChat system includes on four basic types of entities - users, chats, events and
threads.
...
Grouping events in threads allows us to provide the continuous chat experience (i.e. your chats never end) while still maintaining smaller, logical chunks of events (e.g. for reporting and caching purposes).
...
- `GROUPS_OFFLINE` - a group in targeted chat is offline. It can happen for licenses without continuous chats enabled.

## cancelRate

It cancels rate-related thread properties.
...
| arguments | type   | description                            |
| --------- | ------ | -------------------------------------- |
| chat      | string | Chat's id of requested history object. |
...
In most cases, you don't need to use this method directly. Instead, consider using [`getChatHistory`](#getchathistory).
...
This method allows you to fetch a form template for a given group and form type.
...
It sends the request if the SDK is connected or queues the update to perform once the connection gets established.
...
customerSDK.on('chat_transferred', payload => {
  console.log('chat_transferred', payload)
})
```
...
It informs that Customer SDK has recovered from an "unstable" connection state. It's always preceded by the [`"connection_unstable"`](#connection_unstable) event.
...
| chat             | string  | Chat's id of the sent postback             |
| thread           | string  | Thread's id of the sent postback           |
| event            | boolean | Event's id of the sent postback            |
...
Most of the disconnection reasons can be treated as errors because of that they are described in more details in this section.
