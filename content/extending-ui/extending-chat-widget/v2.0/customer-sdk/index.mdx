...
| clientId             | string             |         | Your client id, you receive this value when you register your application in the LiveChat developers console.                                                                |
| autoConnect          | boolean            | true    | Optional; should the library try to reconnect on it's own.                                                                                                                   |
| uniqueGroups         | boolean            | false   | Optional; should chat sessions be separated between groups, this feature is analogous to [setting separate chat sessions][1] in our chat widget.                             |
...
[1]: https://www.livechatinc.com/help/livechat-for-multiple-websites#set-separate-sessions
...
You can use this method to inform that a Customer has seen a greeting. Based on that, the Reports section displays only the greetings seen by Customers instead of all the sent greetings. If a Customer started a chat from a greeting but you didn't execute `acceptGreeting`, the greeting counts as seen in Reports anyway.
...
After using this method, you won't be able to use the destroyed SDK instance.
...
|              |          | lastEventsPerType  | object   | Map of event types to event objects            |
|              |          | lastSeenTimestamps | object   | Map of user IDs to optional lastSeenTimestamps |
...
Returns the summary of your chat threads including the total count of threads, the list of IDs, and a number of events in each thread.
...
| type    | description                                   |
| ------- | --------------------------------------------- |
| promise | Promise resolving to a constant success value |
...
Informs that the Customer SDK has disconnected.
...
| properties | type   |                                                  |
| ---------- | ------ | ------------------------------------------------ |
| url        | string | URL the Customer's currently on                  |
| title      | string | Title of the website the Customer's currently on |
...
- `setCustomerFields` method has been added.
