...
`<script src="https://unpkg.com/@livechat/customer-sdk@2.0.0-beta.1"></script>`
...
- `value` - an object with array of threads, each containing an array of its events, and array of user objects
...
history.next().then(result => {
  ...
  const { threads, users } = result.value
...
customerSDK.on('chat_thread_properties_deleted', payload => {
  ...
  console.log('chat_thread_properties_deleted', { chatId, threadId, properties })
})
```
...
customerSDK.on('chat_thread_properties_updated', payload => {
  const { chatId, threadId, properties } = payload
  console.log('chat_thread_properties_updated', { chatId, threadId, properties })
})
```
...
customerSDK.on('chat_user_added', payload => {
  const { user, chatId, present } = payload
  console.log('chat_user_added', { user, chatId, present })
})
```
...
customerSDK.on('chat_user_removed', payload => {
  ...
  console.log('chat_user_removed', { userId, chatId })
})
```
...
customerSDK.on('event_properties_deleted', payload => {
  ...
  console.log('event_properties_deleted', { chatId, threadId, eventId, properties })
})
```
...
customerSDK.on('event_properties_updated', payload => {
  const { chatId, threadId, eventId, properties } = payload
  console.log('event_properties_updated', { chatId, threadId, eventId, properties })
})
```
...
customerSDK.on('events_marked_as_seen', payload => {
  const { chatId, userId, timetamp } = payload
  console.log('events_marked_as_seen', { chatId, userId, timetamp })
})
```
...
customerSDK.on('incoming_rich_message_postback', payload => {
  const { userId, chatId, threadId, eventId, postback } = payload
...
customerSDK.on('thread_closed', payload => {
  ...
  console.log('thread_closed', { chatId })
})
```
