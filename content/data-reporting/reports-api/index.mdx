...
| `GET` | `/v3/reports/chats/agents_chatting_duration` |
...
Shows the average chatting duration of agnets witihn a license.
...
| `agents`              | no       | agent emails separated by comma; if not specififed, returns the data for all agents witihn the license |
| `groups`              | no       | group IDs separated by comma               |
| `tags`                | no       | names of tags separated by comma           |
| `customer_client_ids` | no       | client IDs separated by comma              |
| `distribution`        | no       | possibe values: `hour`, `day`, `month`; defaults to `day` |
| `tagged`              | no       | possibe values: `true`, `1`, `false`, `0`; defaults to `false` |
...
- `records.day.seconds` - the total duration of all chats `agents` had that `day`

</Text>

<Code>

<CodeSample path={'GET https://api.livechatinc.com/v3/reports/chats/agents_chatting_duration'}>

<Sample>

```shell
curl "https://api.livechatinc.com/v3/reports/chats/agents_chatting_duration?from=2020-09-01T00:00:00%2B02:00&to=2020-09-14T23:59:59%2B02:00" \
  -u john.doe@mycompany.com:c14b85863755158d7aa5cc4ba17f61cb
```
