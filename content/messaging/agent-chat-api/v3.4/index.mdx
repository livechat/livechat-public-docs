...
Authors (except the requester) of all initial events must be listed in the `users` field. The `visibility` of both events and users is set to `all`.
...
| **Required scopes \*** | `chats.conversation--all:rw` `chats.conversation--access:rw`           |
...
The user must be [added to the chat](/messaging/agent-chat-api/rtm-reference/#user_added_to_chat) before they can send events.
Events with `visibility:agents` are sent to agents only, and with `visibility:all` to all users. The user with `visibility:agents` cannot send events with `visibility:all`.
