...
| `previous_accessible_thread_id` | optional  | Present only if there is a preceding thread accessible for user. Only present in [`list_archives`](#list-archives) |
| `next_accessible_thread_id`     | optional  | Present only if there is a following thread accessible for user. Only present in [`list_archives`](#list-archives) |
