...
Pushes are **server - client methods** used for keeping the application state up-to-date. They are available only in the `websocket` transport. The Agent Chat Web API uses [webhooks](/messaging/agent-chat-api/#webhooks). The majority of pushes have their webhook equivalents.
