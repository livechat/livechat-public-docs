...
| `customer_monitoring_level`         | No       | `string`   | Possible values: `my`, `chatting`, `invited`, `online` **\*\***. Defaults to `my` if [login](#login) creates the first session; otherwise it preserves the current `customer_monitoring_level`. |
