...
| `pushes`                            | No       | `object`   | May be used when interested only in specific pushes. By default, it's set to `all` for the version of your currently established RTM connection.                                              |
| `pushes.<version>`                  | Yes      | `[]string` | A list of push subscriptions for a specific version;`<version>` is a version number, without `v` prefix; possible values are names of the pushes                                              |
