...
- There're new fields `previous_thread_id`, `next_thread_id` and `queue` in the [**Thread**](/messaging/agent-chat-api/v3.2/#threads) data structure.
...
- There're new fields `requester_id` and `transferred_from` in [`incoming_chat`](/messaging/agent-chat-api/v3.2/rtm-reference/#incoming_chat).
- [`chat_transferred`](/messaging/agent-chat-api/v3.2/rtm-reference/#chat_transferred) push is no longer tied directly to **Transfer Chat**, can now be generated when the transfer is implicit e.g. the agents are reassigned due to their inactivity.
- There're new fields `requester_id`, `thread_id`, `reason`, `queue` and `transferred_to` in `chat_transferred`.
- `chat_user_added` push was renamed to [`user_added_to_chat`](/messaging/agent-chat-api/v3.2/rtm-reference/#user_added_to_chat).
- There're new fields `reason` and `requester_id` in `user_added_to_chat`.
- `chat_user_removed` push was renamed to [`user_removed_from_chat`](/messaging/agent-chat-api/v3.2/rtm-reference/#user_removed_from_chat).
- There're new fields `reason` and `requester_id` in `user_removed_from_chat`.
...
- Removed fields `type` and `ids` from `chat_transferred`.
- Removed field `user_type` from `user_added_to_chat`.
- Removed field `user_type` from `user_removed_from_chat`.
