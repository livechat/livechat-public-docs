...
- **Get Chat Threads** evolved into **Get Chat** ([Web](/messaging/agent-chat-api/v3.2/#get-chat) & [RTM](/messaging/agent-chat-api/v3.2/rtm-reference/#get-chat)) and allows now to get chat with specific thread or the last one. It also changed it shape: `order` field was removed as well as nesting was simplified (no `chat` key).
