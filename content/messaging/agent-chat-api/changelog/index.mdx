...
- There's a new parameter `away` in the [Login](/messaging/agent-chat-api/v3.2/rtm-reference/#login) request. Controls whether the connection should be considered to be in the away state by the auto-away feature. Setting it to true should help avoiding those pesky chat assignments that happen right after random reconnections.
