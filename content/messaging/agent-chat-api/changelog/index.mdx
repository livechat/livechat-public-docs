...
- The **Chat access** ([Web](/messaging/agent-chat-api/v3.2/#chat-access) & [RTM](/messaging/agent-chat-api/v3.2/rtm-reference/#chat-access)) methods, `grant_access`, `revoke_access`, and `set_access` were renamed, respectively, to `chat_grant_access`, `chat_revoke_access`, and `chat_set_access` along with the corresponding [pushes](/messaging/agent-chat-api/v3.2/rtm-reference/#chat-access-1). These methods no longer accept the `resource` parameter.
