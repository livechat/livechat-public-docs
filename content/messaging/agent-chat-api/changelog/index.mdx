...
- The `order` field was renamed to `sort_order` in **List Chats**.
- The `order` field in the `chat` object was removed from the responses of the **List Chats** method.
...
- There are new fields, `requester_id` and `transferred_from` in the [**incoming_chat**](/messaging/agent-chat-api/v3.2/rtm-reference/#incoming_chat) push.
