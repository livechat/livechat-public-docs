...
- There's new field `queue` in the [**Thread**](/messaging/agent-chat-api/v3.2/#threads) data structure.
- **Transfer Chat** can now be used with inactive chats.
- **Incoming Chat**
  - new field `requester_id`: present if the chat was explicitly transferred by another agent
  - new field `transferred_from`: present only if the chat was transferred, `agent_ids` will be empty if the chat was queued prior to the transfer
- **Chat Transferred**
  - no longer tied directly to **Transfer Chat**, can now be generated when the transfer is implicit e.g. the agents are reassigned due to their inactivity
  - `requester_id` is now optional and will be present only when `reason` is `manual`
  - new field `thread_id`: present only if the chat is active during the transfer
  - new field `reason`: why the chat was transferred,
    - the reason will be set to `manual` if the push results from **Transfer Chat** or the chat was explicitly picked from the queue
  - new field `queue`
  - new field `transferred_to`: replaces `type` - `ids` pair
- **Chat User Added**:
  - renamed to **User Added To Chat**,
  - removed field `user_type`,
  - new field `reason`: why the user was added to the chat,
  - new field `requester_id`: present if the user was added by an agent.
- **Chat User Removed**:
  - renamed to **User Removed From Chat**,
  - removed field `user_type`,
  - new field `reason`: why the user was removed from the chat,
  - new field `requester_id`: present if the user was removed by an agent.

### Removed

- The **Update Agent** method was removed.
- The `agent_updated` push was removed.
- The **Set Chat Access** method was removed. It's use cases are now covered by the **Transfer Chat**.
