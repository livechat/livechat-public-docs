...
| `filters.agents.<filter_type>`                        | No       | `any`     |  `exists` = `false` will return unassigned chats, while `true` will return assigned       |
| `filters.agent_ids`                                   | No       | `array`   | Array of agent IDs (Deprecated, use `agents`)                                             |
