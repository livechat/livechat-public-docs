...
| **Other**       | [`mark_events_as_seen`](#mark-events-as-seen) [`send_typing_indicator`](#send-typing-indicator) [`multicast`](#multicast) [`get_active_chat_count`](#get-active-chat-count)                                                                                                                                                                               |
...
It returns the **total number** of active chats an Agent has. Agent's chats from **all groups** are taken into account.
...
3. Then, it calculates the number of active chats for each of those Agents. 

#### Specifics

|                        |                                                                       |
| ---------------------- | --------------------------------------------------------------------- |
| **Method URL**         | `https://api.livechatinc.com/v3.2/agent/action/get_active_chat_count` |
| **Required scopes**    | -                                                                     |
| **RTM API equivalent** | [`get_active_chat_count`](rtm-reference/#get-active-chat-count)       |
| **Webhook**            | -                                                                     |
...
| Parameter    | Required | Data type | Notes                                                              |
| ------------ | -------- | --------- | ------------------------------------------------------------------ |
| `group_ids`  | Yes      | `array`   | Groups in which you want to search for Agents. Max array size: 200 |
...
```shell
curl -X POST \
https://api.livechatinc.com/v3.2/agent/action/get_active_chat_count \
-H 'Authorization: Bearer <your_access_token>' \
-H 'Content-Type: application/json' \
-d '{
    "group_ids": [0,1,2,3]
