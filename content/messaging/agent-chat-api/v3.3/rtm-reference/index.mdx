...
| `customer_push_level`               | No       | `string`  | Possible values: `my`, `engaged`, `online` __**__. Defaults to `my` if [login](#login) creates the first session; otherwise it preserves the current `customer_push_level`. |
...
* `engaged` - all customers that aren't just browsing the website (for example, they're chatting or being invited) within groups the agent is assigned to
