...
To prevent sending a massive number of requests, there are rate limits for methods' calls. After exceeding them, the requester will get a `Too many requests` error in a response to a specific request. After some time, the request will be unblocked so it can be sent again.
...
| `too_many_requests`         | Too many requests             | A request has been rate limited; after some time it will be unblocked automatically  |
