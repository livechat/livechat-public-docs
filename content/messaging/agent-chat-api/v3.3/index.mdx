...
Basing on `limit` and `sort order` parameters pagination is able to decide how many records will be returned at once and if it should fetch oldest or newest data first.

Any filters that could be applied, should be provided in first pagination request. In response we can get `next_page_id` and `previous_page_id` parameters.
Next request should be performed using one of these parameters as `page_id` depending on iterations direction - forward or backward.

`filters`, `limit` and `sort_order` parameters can not be provided along with `page_id`.
...
<Errors errors="authentication,authorization,entity_too_large,internal,license_expired,license_not_found,limit_reached,misdirected_request,request_timeout,service_unavailable,unsupported_version,validation,wrong_product_version,chat_inactive,too_many_requests"/>
