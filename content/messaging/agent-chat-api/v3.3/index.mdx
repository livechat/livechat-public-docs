...
Marks a customer as followed. All changes to the customer will be sent to the requester until the customer leaves the website or is unfollowed. The customers in the requester's chats don't need to be followed. The requester will receive all customer pushes regardless of their follower status.

If the agent is following a customer and the customer's access is outside of the agent's access, the follower status itself will not be broken (ie. the agent will still be following the customer) but the agent won't receive pushes about the customer. Once the customer's access matches the agent's access, the pushes will resume.
...
Removes the requester from the customer followers. Calling this method on a customer the requester is chatting with will result in success, however, the requester will still receive pushes about customer's data updates. The unfollow will take effect once the chat ends.
...
<Errors errors="authentication,authorization,entity_too_large,internal,license_expired,license_not_found,misdirected_request,request_timeout,unsupported_version,validation,wrong_product_version,chat_inactive,too_many_requests"/>
