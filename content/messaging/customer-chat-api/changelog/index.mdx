...
- There's new field `queue` in the [**Thread**](/messaging/customer-chat-api/v3.2/#threads) data structure.
- **Incoming Chat**
  - new field `requester_id`: id of a user that activated the chat
- **Chat Transferred**
  - no longer tied directly to **Transfer Chat** ([Web](/messaging/agent-chat-api/v3.2/#transfer-chat) & [RTM](/messaging/agent-chat-api/v3.2/rtm-reference/#transfer-chat)), can now be generated when the transfer is implicit e.g. the agents are reassigned due to their inactivity
  ...
  - new field `reason`: why the chat was transferred,
    - the reason will be set to `manual` if the push results from **Transfer Chat** or the chat was explicitly picked from the queue by an agent
  - new field `queue`
