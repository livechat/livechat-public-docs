...
Requests verification of customer's email address by sending them verification email with link to confirm identity.

Once Customer verifies their identity, a POST callback is sent to URL provided in `callback_uri` request parameter with verified email in payload.

Identity verification mechanism has following restraints:
- you can stack multiple verification requests, but only 10 least recently requested callbacks will be triggered
- after 7 days since last verification request, all callbacks are voided and will not be sent
- if Customer already has their email verified, callback will be sent immediately without prompting Customer to verify again
...
| Parameter      | Required | Data type | Notes                                                                                                       |
| -------------- | -------- | --------- | ----------------------------------------------------------------------------------------------------------- |
| `callback_uri` | Yes      | `string`  | URI to be called after Customer confirms their email address. Should return 200 code. There are no retries. |
...
```shell
curl -X POST \
  'https://api.livechatinc.com/v3.2/customer/action/request_email_verification?license_id=<license_id>' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <customer_access_token>' \
  -d '{
