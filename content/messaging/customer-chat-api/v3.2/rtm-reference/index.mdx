...
This document describes the **Customer Chat Real-Time Messaging API v3.2**.
...
| Field                   | Sending    | Returned    | Data type               |  Notes                                                                                                   |
| ------------------------| :--------: | ----------- | ----------------------- | -------------------------------------------------------------------------------------------------------: |
| `id`                    | N/A        | always      | `string`                | Generated server-side                                                                                    |
| `custom_id`             | optional   | optionally  | `string`                |                                                                                                          |
| `created_at`            | N/A        | always      | `string`                | Generated server-side; Date & time format with a resolution of microseconds, `UTC string`.               |
| `type`                  | required   | always      | `string`                | `filled_form`                                                                                            |
| `author_id`             | N/A        | always      | `string`                | Generated server-side                                                                                    |
| `recipients`            | required   | always      | `string`                | Possible values: `all` (default), `agents`                                                               |
| `properties`            | optional   | optionally  | [`object`](#properties) |                                                                                                          |
| `form_id`               | ?          | ?           | `string`                | ?                                                                                                        |
| `fields`                | required   | always      | `array of objects`      | The fields a form contains.                                                                              |
| `fields.type`           | required   | always      | `string`                | Possible values: `checkbox`, `email`, `name`, `question`, `textarea`, `group_chooser`, `radio`, `select` |
| `fields.id`             | ?          | ?           | `string`                | For all field types                                                                                      |
| `fields.label`          | required   | always      | `string`                | For all field types                                                                                      |
| `fields.answer`         | N/A        | optionally  | `any`                   | For `email`, `name, `radio`, and `group_chooser`                                                         |
| `fields.answer.id`      | N/A        | optionally  | `string`                | For `email`, `name, `radio`, and `group_chooser`                                                         |
| `fields.answer.label`   | required   | always      | `string`                | For `email`, `name, `radio`, and `group_chooser`                                                         |
| `fields.answer.group_id`| required   | always      | `number`                | For `group_chooser`                                                                                      |
| `fields.answers`        | required   | always      | `array of objects`      | For `checkbox`                                                                                           |
| `fields.answers.id`     | required   | always      | `string`                | For `checkbox`                                                                                           |
| `fields.answers.label`  | required   | always      | `string`                | For `checkbox`                                                                                           |
...
| **Customers**  | [`update_customer`](#update-customer) [`update_customer_page`](#update-customer-page) [`set_customer_fields`](#set-customer-fields) [`get_customer`](#get-customer)                                                                                                                                                                                                                     |
| **Status**     | [`login`](#login) [`get_groups_status`](#get-groups-status)                                                                                                                                                                                                                                                                                             |
| **Other**      | [`get_form`](#get-form) [`get_predicted_agent`](#get-predicted-agent) [`get_url_details`](#get-url-details) [`mark_events_as_seen`](#mark-events-as-seen) [`accept_greeting`](#accept-greeting) [`reject_greeting`](#reject-greeting)                                                                                                                                                                                              |
...
### Reject Greeting
Rejects a greeting (an invitation to the chat). For example, Customers could reject greetings by minimizing the chat widget with a greeting.

#### Specifics

|                        |                                              |
| ---------------------- | -------------------------------------------- |
| **Action**             | `reject_greeting`                            |
| **Web API equivalent** | [`reject_greeting`](../#reject-greeting)     |
| **Push message**       | [`greeting_cancelled`](#greeting_cancelled)  |

#### Request

| Parameter    | Required | Data type   | Notes                         |
| ------------ | -------- | ----------- | ----------------------------- |
| `unique_id`  | Yes      | `string`    | ID of the greeting to reject. You can get it from the [`incoming_greeting`](#incoming_greeting) push. |
...
| **Other**       | [`incoming_typing_indicator`](#incoming_typing_indicator) [`incoming_multicast`](#incoming_multicast) [`events_marked_as_seen`](#events_marked_as_seen) [`incoming_greeting`](#incoming_greeting) [`greeting_cancelled`](#greeting_cancelled)                                                                                                                           |
...
### `incoming_greeting`
It's sent when all the conditions to send a [greeting](https://www.livechatinc.com/help/why-should-i-set-up-chat-greetings/) are met.
...
```json
{
  "action": "incoming_greeting",
    ...
    "payload": {
        ...
        "text": "Hello, would you like to talk about our products?",
        "displayed_first_time": true,
        "agent": {
            "name": "Agent Name",
            "id": "b5657aff34dd32e198160d54666df9d8",
            "avatar": "https://domain.com/image25.png",
            "job_title": "Support Agent",
            "is_bot": false
        }
    }
