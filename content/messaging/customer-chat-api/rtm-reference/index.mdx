...
Rich message event contains rich message data structure. Rich messages are so powerful that we dedicated them a [separate doc](/extending-ui/extending-chat-widget/rich-messages/).
...
- They notify you when specific events are triggered. 
- Available only in the websocket transport. 
- Instead of pushes, Web API uses [webhooks](/messaging/customer-chat-api/#webhooks). 
- RTM API actions can also trigger webhooks. 
