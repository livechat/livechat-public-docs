...
You need to pass the access token to the `init()` method.
...
```js
const chat = new ChatSDK({ account_token: "dal:MpubEGdISJCCiFelap3Qtg", debug: true })
```
...
```js
const chat = new ChatSDK({ account_token: "dal:MpubEGdISJCCiFelap3Qtg", region: "europe" })
```
...
```js
const chat = new ChatSDK({ account_token: "dal:MpubEGdISJCCiFelap3Qtg", apiVersion: "3.0" })
```
...
| Argument   | Required | Data type | Notes                                         |
| ---------- | -------- | --------- | --------------------------------------------- |
...
To show you **Chat SDK** in action, we've prepared a sample app, **Simple Agent App** . It's primary function is to send text messages. Apart from that, it gives you access to previous conversations, 
...
1. [Create an app](/getting-started/guides/#creating-livechat-apps) in Developer Console. You need it to get a **Client Id**. Once the app is created, you can find it the **Client Id** in 
**Building Blocks -> Authorization** in Developer Console.

2. Paste the **Client Id** into the `.env` file.

3. Fire **Simple Agent App** with the following command:

` `

To start a chat, log in to you LiveChat account and choose the **Preview live** option available in the **Settings** tab. You'll now be able to receive messages and respond to them from within **Basic Agent App**.

It's worth mentioning that all functions invoked before loggging in are queued. Once you're logged in, they are executed in the same order as they were invoked.  
