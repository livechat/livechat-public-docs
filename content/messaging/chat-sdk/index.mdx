...
- [Messaging APIs in practice](/getting-started/guides/messaging-apis-in-practice/)
  You need to include the access token in the configuration object.

<CodeSample>

```js
const chat = new ChatSDK({ account_token: 'dal:MpubEGdISJCCiFelap3Qtg' })
```
...
```js
const getChatThreadsSummary = (payload) => chatSDK.methodFactory({
    action: 'get_chat_threads_summary',
    payload: {"chat_id": chat_id}
})

getChatThreadsSummary(chat_id)
    .then(data => {
