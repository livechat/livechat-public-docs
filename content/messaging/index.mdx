...
In general, actions invoked by the use of Messaging APIs result in events. You can be notified about those events with **webhooks**. LiveChat provides a number of webhooks, which you can register and manage via the [Configuration API v3](../management/configuration-api/#webhooks).
