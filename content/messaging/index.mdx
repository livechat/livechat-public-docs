...
Watch a short video that illustrates the basic flow between services at LiveChat.

<video width="750" height="500" style="height: auto;" style="border-bottom: 20px solid white;" controls>
  <source type="video/mp4" src="images/livechat-messaging-api-flow.mp4" />
</video>

For more technical details, read the description below.

![Basic flow between services at LiveChat](images/basic-flow-between-services-livechat.png)

The LiveChat communication protocol uses *websockets*. They act like tubes that connect two parts and enable transport between them. In the basic model of the LiveChat services, there are two websocket connections:

- Agent App --- LiveChat's backend
- LiveChat's backend --- Chat Widget
...
Apart from pushes, actions can trigger the LiveChat's backend to also generate webhooks. To receive a webhook, you need to [register it](/management/configuration-api/#register-webhook) first. That's how you let the LiveChat's backend know you want to be notified about certain actions.
