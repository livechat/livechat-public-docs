...
Additionally, whenever payment is due and an invoice is issued, the charge enters the `past_due` status. As soon as the invoice is paid the status changes back to `active`.
In the happy case this only takes a few seconds. In other cases the `past_due` status may be used to e.g. display a pending invoice notification to the app user, or limit the app functionality.
