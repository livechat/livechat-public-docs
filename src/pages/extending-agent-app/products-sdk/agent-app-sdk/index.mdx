...
3. **Monitor events**: Set up listeners for transactions related events. You can listen to available [app webhooks](/guides/console-guides/app-webhooks) or built-in [transaction events](/extending-agent-app/products-sdk/agent-app-sdk#events-3) from the Agent App SDK.
...
You can leverage the OAuth 2.1 authorization flow to use all of LiveChat APIs. Head to [Sign in with LiveChat](/authorization/oauth-authorization#sign-in-with-livechat) docs for more information.
