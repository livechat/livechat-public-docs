...
5. Go to **Building blocks** and add a new **HelpDesk Widgets** block.
    ...
    - Choose the **widget placement**. It allows you to decide where exactly in the HelpDesk App you want to display your integration. Currently, the possible locations are the Details section, Fullscreen apps, or Settings. [Read more](/extending-agent-app#app-locations) about app locations.  
...
```
npm install @helpdesk/helpdesk-sdk
```
...
```js
import * as HelpDesk from "@helpdesk/helpdesk-sdk";
```
...
```js
const HelpDesk = require("@helpdesk/helpdesk-sdk");
```
...
```js
import { createDetailsWidget } from "@helpdesk/helpdesk-sdk";
...
```js
widget.on("ticket-details", profile => {
...
If you want to access the current ticket, you should use the `getTicketDetails` method.
...
```js
const profile = widget.getTicketDetails();
```

</CodeSample>

The returned profile will be an object identical to the one emitted by the `tickets_details` event. It can also be `null` if no profile was recorded.
...
If you need a quick way to **deploy** your widget, you can use [Netlify Drop](https://developers.livechat.com/blog/widget-hosting-on-netlify/).
...
<div style={{maxWidth: '800px', marginBottom: '30px'}}>
  <img src="/images/extending-agent-app/livechat-apps-helpdesk-widgets-opening-preview-state-how-to-add-a-widget.png" alt="HelpDesk Widgets" width="700px" height="944px"/>
</div>
...
If you have any feature requests related to the Agent App Widgets, let us know! We're open for your insights and suggestions. Feel free to drop us an email at [developers@livechat.com](mailto:developers@livechat.com).