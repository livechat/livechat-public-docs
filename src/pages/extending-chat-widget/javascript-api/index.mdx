...
It sends an e-commerce event to LiveChat. Cart and order events appear in real time in the agent's Inbox; order events also feed the **All Sales** and **AI Agent Sales** reports (data syncs once a day, so orders appear in the reports the following day).
...
| data*    | `object` | event payload — shape depends on `name`                                  |
...
| name                              | when to fire                                            |
| --------------------------------- | ------------------------------------------------------- |
| `store_order_was_created`         | order completed (fires on the order confirmation page)  |
| `store_product_added_to_cart`     | item added to the cart                                  |
| `store_product_removed_from_cart` | item removed from the cart                              |
| `store_product_page_viewed`       | product detail page rendered                            |

The sections below detail each event's `data` shape, with one example showing every supported field.

### `store_order_was_created`

Fire on your order confirmation page after a purchase is completed. Order events feed both the agent's Inbox (real time) and the All Sales / AI Agent Sales reports (next day).
...
Fire when a customer adds an item to the cart.
...
Fire when a customer removes an item from the cart.
...
### `store_product_page_viewed`

Fire on each product detail page load. Used for browsing-funnel analytics — doesn't render in the chat feed.

#### Example

```js
LiveChatWidget.call("track_sales_event", {
  name: "store_product_page_viewed",
  data: {
    customer_event_id: crypto.randomUUID(),
    product: {
      product_id: "prod_221",
      variant_id: "var_441",
    },
...
| param                  | type     | description                                                       |
| ---------------------- | -------- | ----------------------------------------------------------------- |
| `customer_event_id`*   | `string` | UUID v4, unique per page view                                     |
| `product.product_id`*  | `string` | your platform's product ID                                        |
| `product.variant_id`*  | `string` | variant identifier — use `product_id` when there are no variants  |
