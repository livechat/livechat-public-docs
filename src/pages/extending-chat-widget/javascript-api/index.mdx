...
| param     | type       | description                                                                          |
| --------- | ---------- | ------------------------------------------------------------------------------------ |
| platform  | `"custom"` | fixed to `"custom"` for custom-store integrations |
...
| items*    | `array`    | the items in the customer's cart line                        |
...
| `items[].title`*              | `string` | product name displayed to the agent                                              |
| `items[].qty`*                | `number` | product quantity — must be an integer greater than 0                                |
| `items[].value`*              | `number` | cart line total after discounts (unit price × qty − discounts), not a unit price |
| `items[].variantId`           | `string` | variant identifier from your platform                                       |
| `items[].variantTitle`        | `string` | human-readable variant description — hidden when empty or when the value is `"Default Title"` |
| `items[].sku`                 | `string` | stock-keeping unit                                                           |
| `items[].thumbnailUrl`        | `string` | product image URL — if omitted, a placeholder icon is used                 |
| `items[].productPreviewUrl`   | `string` | link to the product page — if omitted, the "View in store" button will be hidden |
| `items[].discounts[].amount`  | `string` | cart line-level discount amount                                                  |
