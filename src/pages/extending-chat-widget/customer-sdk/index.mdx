...
`<script src="https://unpkg.com/@livechat/customer-sdk@3.0.0"></script>`
...
| identityProvider     | () => CustomerAuth |         | Optional; allows for providing own instance of CustomerAuth object which contains customer access token handlers. See [Custom Identity Provider](https://developers.livechat.com/docs/extending-chat-widget/custom-identity-provider) for more information. |
...
- `GROUP_UNAVAILABLE` - there are no available slots in the requested group, so it is not possible to return a predicted Agent for it. If you call `startChat` or `resumeChat` accordingly, you are most likely going to end up in the queue.
...
- Handle new event type `'form'` representing optional custom forms sent from the server
