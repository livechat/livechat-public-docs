...
Customer SDK allows you to create frontend apps, so if you're looking for server-side solutions, you should explore the [LiveChat Customer Chat API](https://platform.text.com/docs/messaging/customer-chat-api/) instead.
If you want to dive deeper into the LiveChat Platform, you might find the [Platform Overview](https://platform.text.com/docs/getting-started/) article handy.
...
[Developer Console](https://platform.text.com/console) (select the _Web app (frontend,
...
For the time being you need to register your application in the <a href="https://platform.text.com/console" target="_blank">Developers Console</a>
...
| autoConnect          | boolean            | true    | Optional; decides whether the SDK should reconnect on its own.                                                                                                                                                                                                        |
...
| identityProvider     | () => CustomerAuth |         | Optional; allows for providing own instance of the CustomerAuth object which contains the customer access token handlers. See [Custom Identity Provider](https://platform.text.com/docs/extending-chat-widget/custom-identity-provider) for more information. |
...
You can read more about key concepts of the LiveChat messaging in the [Messaging Overview](https://platform.text.com/docs/messaging/#key-concepts).
...
| data.chat.properties        | object  | Optional; [default properties docs](https://platform.text.com/docs/messaging/references/default-properties) |
...
| data.chat.thread.properties | object  | Optional; [default properties docs](https://platform.text.com/docs/messaging/references/default-properties) |
...
| chat.thread.properties | object  | Optional; [default properties docs](https://platform.text.com/docs/messaging/references/default-properties) |
...
| properties | object | [Default properties docs](https://platform.text.com/docs/messaging/references/default-properties) |
...
Indicates that thread has been closed.
...
Indicates that Customer SDK has recovered from an "unstable" connection state. It's always preceded by the [`"connection_unstable"`](#connection-unstable) event.
...
Indicates that Customer SDK has detected that connection quality is poor. It doesn't mean that it has disconnected from the server just yet.
...
Indicates that the customer identity has changed, typically due to browser privacy features blocking authentication cookies. The SDK handles this by clearing sensitive session data and switching to the new identity.
...
Indicates that an AI agent is currently thinking or processing a request.
The push payload contains information about the thinking state, including an optional custom title and description.
...
| chatId      | string | Chat ID                                                     |
| threadId    | string | Thread ID                                                   |
| authorId    | string | The ID of the agent that is thinking          |
...
Indicates a welcome message received in response to [`requestWelcomeMessage`](#requestwelcomemessage).
...
Indicates an event preview before it's sent to the chat. Only text events are supported for preview.
...
The Customer SDK handles identity changes by:

- Automatically switching to the new customer identity
- Clearing sensitive session data (chats, cached user data) for security
- Maintaining the SDK functionality without crashes
- Emitting an `identity_changed` event so you can respond to identity updates.
...
The subscription associated with the specified organization ID has expired. Ensure there are no unpaid invoices for this subscription.

## License not found

The provided organization ID doesn't exist. Check the options passed to the Customer SDK and ensure the parameters are correct.
...
- Switched to the Customer Chat API v3.6.
