...
<Code>
<CodeSample path={'START WITH MOMENTS APP USING NPM'}>
...
<Code>
<CodeSample path={'IMPORT CREATE MOMENTS SDK'}>
...
<Code>
<CodeSample path={'USING SCRIPT TAG'}>
...
<Code>
<CodeSample path={'ASSIGNING FUNCTION'}>
...
<Code>
<CodeSample path={'SEND SYSTEM MESSAGE'}>
...
<Code>
<CodeSample path={'SET ATTRIBUTES'}>
...
<Code>
<CodeSample path={'SET EXTERNAL LINK'}>
...
Set `isFragile` value. It can be provided as part of initialization data and changed later with this function.
Parameters:

-   `value` - Boolean

<Code>
<CodeSample path={'SET ISFRAGILE'}>
...
Update customer's name or email.
Parameters:
...
<Code>
<CodeSample path={'UPDATE CUSTOMER DATA'}>
...
-   `licenseId` - ID of the license for which a Moment has been initialized
-   `chatId` - ID of a customer's chat (returns `undefined` if there's no chat)
-   `groupId` - ID of a customer's group
...
- New LauncherData property: `groupId`
