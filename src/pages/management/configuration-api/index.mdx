...
| `rules[].urls[].url`                      | No       | `string`   | The URL for the `url_funnel`<sup>**1**</sup> and `url_match_page_view_number`<sup>**1**</sup> rule. Required when `urls` array is used.                    |
...
| `url_match_page_view_number`  | `value`, `operator`, `urls` | The `value` contains the number of pages viewed to compare. The `urls` array with 1-10 URL objects, each with `url` and `operator`. The rule matches if any of the `urls` matches. |
