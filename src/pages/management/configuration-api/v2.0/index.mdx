...
To start using OAuth 2.1, please read a dedicated [Authorization](/authorization/oauth-authorization/) guide.
...
You can create a PAT from <a href="https://platform.text.com/console/" target="_blank">Developer Console</a>. For more guidelines on creating PATs, check out [this tutorial](/authorization/personal-access-tokens).
...
Available only for licenses with new monitoring solution enabled. The greeting will be sent when the customer visits
the page containing ad provider parameters for the first time (per session).
