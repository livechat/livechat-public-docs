...
Please note that this documentation refers to the **API version 2.0**. This is the **deprecated** version. We encourage you to migrate to the latest stable version. [Read more about versioning...](/management/#api-versioning)
...
You can create a PAT from <a href="https://platform.text.com/console/" target="_blank">Developer Console</a>. For more guidelines on creating PATs, check out [this tutorial](/authorization/authorizing-api-calls/#personal-access-tokens).
...
💡 When sending authorization credentials in a header, the *email:PAT* string has to be **base64-encoded**. Alternatively, enter your credentials in the Authorization tab in Postman, which will automatically encode them for you (see the image below).
  ...
  <img src="/images/management/livechat-basic-authorization-postman.png" alt="Enter LiveChat auth credentials in Postman" width="1700px" height="400px"  />
...
**Please note this API is deprecated**. We highly recommend switching to the new [Management APIs](/management/configuration-api#agents).
...
We highly recommend switching to the new [List Agents](/management/configuration-api#list-agents) method.
...
💡 Not sure which protocol you use? You use the old one if you don't see the [new scopes](/authorization/scopes/#agent-scopes) in <Placeholder id="DEVELOPER_CONSOLE_URL" />.
...
We highly recommend switching to the new [Get Agent](/management/configuration-api#get-agent) method.
...
We highly recommend switching to the new [Create Agent](/management/configuration-api#create-agent) method.
...
We highly recommend switching to the new [Update Agent](/management/configuration-api#update-agent) method.
...
We highly recommend switching to the new [Delete Agent](/management/configuration-api#delete-agent) method.
...
We highly recommend switching to the new [List Chats](/messaging/agent-chat-api#list-chats) method.
...
We highly recommend switching to the new [Get Chat](/messaging/agent-chat-api#get-chat) method.
...
We highly recommend switching to the new [Tag Thread](/messaging/agent-chat-api#tag-thread) and [Untag Thread](/messaging/agent-chat-api#untag-thread) methods.
...
| `ads_traffic`       | The greeting will be sent when customer fist visited page contains ad provider parameters                           |
...
The greeting will be sent when the customer first visit url contains specified ads parameters.
...
the customer first visit url contains `fbclid=1234`.
You also can add the optional `operator` parameter (defaults to `equals`).

Possible `variable_name` values are:
<ul>
  <li><code>facebook</code> for <code>fbclid</code>,</li>
  <li><code>google</code> for <code>dclid</code>, <code>gbraid</code>, <code>gclid</code>, <code>gclsrc</code>, <code>
  wbraid</code>,</li>
  <li><code>microsoft</code> for <code>msclkid</code>,</li>
  <li><code>twitter</code> for <code>twclid</code>,</li>
  <li><code>yahoo</code> for <code>yclid</code>,</li>
</ul>

Max allowed `variable_value` length is 255 characters.

When not providing `variable_name` and `variable_value`, the greeting will be sent when the customer first visit url
contains any of the ads parameters.

When providing only `variable_name`, the greeting will be sent when the customer first visit url containing any of the
ads parameters coming from the provider with the specified name.

When providing only `variable_value`, the greeting will be sent when the customer first visit url contains any of the
ads parameters with the specified value.
</Text>
...
curl "https://api.livechatinc.com/greetings\
  ...
  -H "Authorization: Basic john.doe@mycompany.com:<your_PAT>" \ \
  -H X-API-Version:2 \
  -d "name=ads+traffic+type+greeting&\
rules[0][type]=ads_traffic&\
rules[0][variable_name]=facebook&\
rules[0][variable_value]=1234&\
rules[0][operator]=contains&\
properties[greeting-message_text]=Hello,+would+you+like+to+talk+about+our+products?"
```
...
**Please note this API is deprecated**. We highly recommend switching to the new [Management APIs](/management/configuration-api#groups).
...
We highly recommend switching to the new [List Groups](/management/configuration-api#list-groups) method.
...
💡 Not sure which protocol you use? You use the old one if you don't see the [new scopes](/authorization/scopes/#group-scopes) in <Placeholder id="DEVELOPER_CONSOLE_URL" />.
...
We highly recommend switching to the new [Get Group](/management/configuration-api#get-group) method.
...
We highly recommend switching to the new [Create Group](/management/configuration-api#create-group) method.
...
We highly recommend switching to the new [Update Group](/management/configuration-api#update-group) method.
...
We highly recommend switching to the new [Delete Group](/management/configuration-api#delete-group) method.
...
**Please note this API is deprecated**. We highly recommend switching to the new [Messaging APIs](/messaging/agent-chat-api#status).
...
We highly recommend switching to the new [List Routing Statuses](/messaging/agent-chat-api#list-routing-statuses) method.
...
**Please note this API is deprecated**. We highly recommend switching to the new [Management APIs](/management/configuration-api#tags).
...
We highly recommend switching to the new [List Tags](/management/configuration-api#list-tags) method.
...
We highly recommend switching to the new [Create Tag](/management/configuration-api#create-tag) method.
...
We highly recommend switching to the new [Delete Tag](/management/configuration-api#delete-tag) method.
...
**Please note this API doesn't have its newer equivalent in LiveChat**. We highly recommend migrating to <a href="https://www.helpdesk.com/" target="_blank">HelpDesk</a>. HelpDesk API Documentation can be found <a href="https://api.helpdesk.com/docs" target="_blank">here</a>.
...
**Please note that this method doesn't have its newer equivalent in LiveChat**. We highly recommend migrating to <a href="https://www.helpdesk.com/" target="_blank">HelpDesk</a>. HelpDesk API Documentation can be found <a href="https://api.helpdesk.com/docs" target="_blank">here</a>.
...
We highly recommend switching to the new [Management APIs](/management/configuration-api#webhooks).
...
We highly recommend switching to the [List Webhooks](/management/configuration-api#list-webhooks) method.
...
We highly recommend switching to the [Register Webhook](/management/configuration-api#register-webhook) method.
...
We highly recommend switching to the the [Unregister Webhook](/management/configuration-api#unregister-webhook) method.
