...
| Required scopes                  | `accounts--all:rc` `accounts--all:rw`<sup>**1**</sup> `subscription.lc:rw`<sup>**1**</sup> `accounts.roles.lc--all:rw`<sup>**2**</sup> |
| [Batch support](#batch-requests) | Yes                                                                                                                                    |

**1)** – to create agents who don't have to wait for approval after creation<br/>
**2)** – to add a `viceowner` to the call. Required when creating an agent with a role different than `normal` (default)
