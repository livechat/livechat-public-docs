...
❗️ This method is not yet available for public use. For now, an agent can only be approved via the LiveChat Agent application UI. We would love to hear how you'd like to use this method -- please send us an email at developers@livechat.com describing your use case.
...
Bot enables writing integrations using the **Agent Chat API** - [Web](/messaging/agent-chat-api/) - to communicate in chats similarly to regular Agents.

A Bot has its own auth token, generated by [Issue Bot Token](#issue-bot-token) method. Each Bot is a resource owned by an application in Developers Platform, identified by its own `client_id`.
...
Bots based on the template will be automatically created on the license when the application is installed. The bots will have the same ID as the bot template. If the application is already installed on the license, the bots will be created only if `affect_existing_installations` is set to `true`.

The bots based on templates will work in the same way as those created by the [Create Bot](#create-bot) method.
...
| `owner_client_id`<sup>**2**</sup>        | Yes      | `string`   | Required only when authorizing via [PATs](/authorization/authorizing-api-calls/#personal-access-tokens). When you provide this param while authorizing with a Bearer Token, the `client_id` associated with the Bearer Token will be ignored, and provided `owner_client_id` will be used instead. |
...
The token has following scopes that allow chatting: `chats--access:rw`, `agents-bot--my:rw`, `customers:rw`.
...
<CodeResponse type="configuration" version="v3.6" json="listGroupProperties"/>
