...
Registers a webhook for the Client ID (application) provided in the request. One Client ID can register up to three webhooks for every `action`. The `license` webhooks'll be enabled automatically. The [Disable License Webhooks](#disable-license-webhooks) method should be called explicitly when there is a need to disable some webhooks.
