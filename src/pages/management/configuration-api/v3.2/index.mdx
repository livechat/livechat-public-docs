...
This document describes the **Configuration API v3.2**. This is the **deprecated** version. We encourage you to migrate to the latest stable version. [Read more about versioning...](/management/#api-versioning)
...
Unlike Agents,  don't have passwords or emails - you cannot log in as a bot.
...
| `default_group_priority` <sup>**5**</sup> | No       | `string`   | The default routing priority for a group without defined priority.                                                                     |
...
| `work_scheduler`                          | No       | `object`   | Work scheduler options to set for the new bot. <sup>**2**</sup>                                                                        |
...
| `work_scheduler`                          | No       | `object`   | Work scheduler options to set for the new bot. <sup>**3**</sup>                                                         |
| `work_scheduler.****.<work_scheduler>`    | No       | `object`   | `<work_scheduler>` values <sup>**5**</sup>.                                                                             |
...
| `<property_name>.domain` <sup>**1**</sup>                  | No                  | `[<type>]` | Array of values that properties can be set to.                                        |
| `<property_name>.range` <sup>**1**</sup>                   | No                  | `object`   | Range of values that properties can be set to.                                        |
...
| Required scopes<sup>**1**</sup> | -                                                                                 |

**1)** If the requester's Client ID matches the namespace, setting license properties is always allowed.
...
**1)** If the requester's Client ID matches the namespace, retrieving license properties is always allowed.
...
**1)** If the requester's Client ID matches the namespace, deleting license properties is always allowed.
Otherwise, the permission depends on how the property was configured during [registration](#register-properties); parameter: `<property_name>.locations.<location>.access.<user>.write`.
...
**1)** If the requester's Client ID matches the namespace, setting group properties is always allowed.
Otherwise, the permission depends on how the property was configured during [registration](#register-properties); parameter: `<property_name>.locations.<location>.access.<user>.write`.
...
**1)** If the requester's Client ID matches the namespace, retrieving group properties is always allowed.
Otherwise, the permission depends on how the property was configured during [registration](#register-properties); parameter: `<property_name>.locations.<location>.access.<user>.read`.
...
**1)** If the requester's Client ID matches the namespace, deleting group properties is always allowed.
Otherwise, the permission depends on how the property was configured during [registration](#register-properties); parameter: `<property_name>.locations.<location>.access.<user>.write`.
...
| Parameter                                | Required | Data type  | Notes                                                                                                                                                    |
| ---------------------------------------- | -------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `action`<sup>**1**</sup>                 | yes      | `string`   | The [action](#triggering-actions) that triggers sending a webhook.                                                                                       |
...
| `additional_data`                        | no       | `[]string` | Additional data arriving with the webhook. Value: `chat_properties` (all actions except for `agent_status_changed`, `agent_deleted`, `customer_created`) |
...
| -------------------------------------------------------------------------------------------- | ------------------------------------------------: |
| [`incoming_chat`](/management/webhooks/v3.2#incoming_chat)                                   |                `chat_member_ids`, `only_my_chats` |
| [`chat_deactivated`](/management/webhooks/v3.2#chat_deactivated)                             |                `chat_member_ids`, `only_my_chats` |
| [`chat_access_granted`](/management/webhooks/v3.2#chat_access_granted)                       |                `chat_member_ids`, `only_my_chats` |
| [`chat_access_revoked`](/management/webhooks/v3.2#chat_access_revoked)                       |                `chat_member_ids`, `only_my_chats` |
| [`user_added_to_chat`](/management/webhooks/v3.2#user_added_to_chat)                         |                `chat_member_ids`, `only_my_chats` |
| [`user_removed_from_chat`](/management/webhooks/v3.2#user_removed_from_chat)                 |                `chat_member_ids`, `only_my_chats` |
| [`incoming_event`](/management/webhooks/v3.2#incoming_event)                                 | `chat_member_ids`, `author_type`, `only_my_chats` |
| [`event_updated`](/management/webhooks/v3.2#event_updated)                                   | `chat_member_ids`, `author_type`, `only_my_chats` |
| [`incoming_rich_message_postback`](/management/webhooks/v3.2#incoming_rich_message_postback) |                `chat_member_ids`, `only_my_chats` |
| [`chat_properties_updated`](/management/webhooks/v3.2#chat_properties_updated)               |                `chat_member_ids`, `only_my_chats` |
| [`chat_properties_deleted`](/management/webhooks/v3.2#chat_properties_deleted)               |                `chat_member_ids`, `only_my_chats` |
| [`thread_properties_updated`](/management/webhooks/v3.2#thread_properties_updated)           |                `chat_member_ids`, `only_my_chats` |
| [`thread_properties_deleted`](/management/webhooks/v3.2#thread_properties_deleted)           |                `chat_member_ids`, `only_my_chats` |
| [`event_properties_updated`](/management/webhooks/v3.2#event_properties_updated)             |                `chat_member_ids`, `only_my_chats` |
| [`event_properties_deleted`](/management/webhooks/v3.2#event_properties_deleted)             |                `chat_member_ids`, `only_my_chats` |
| [`thread_tagged`](/management/webhooks/v3.2#thread_tagged)                                   |                `chat_member_ids`, `only_my_chats` |
| [`thread_untagged`](/management/webhooks/v3.2#thread_untagged)                               |                `chat_member_ids`, `only_my_chats` |
| [`routing_status_set`](/management/webhooks/v3.2#routing_status_set)                         |                                                 - |
| [`agent_deleted`](/management/webhooks/v3.2#agent_deleted)                                   |                                       `agent_ids` |
| [`customer_created`](/management/webhooks/v3.2#customer_created)                             |                                                 - |
| [`events_marked_as_seen`](/management/webhooks/v3.2#events_marked_as_seen)                   |                `chat_member_ids`, `only_my_chats` |
