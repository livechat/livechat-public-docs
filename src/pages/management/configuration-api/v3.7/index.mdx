...
| **Customer bans**    | [`ban_customer`](#ban-customer) [`list_customer_bans`](#list-customer-bans) [`unban_customer`](#unban-customer)                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **Forms**            | [`get_form`](#get-form) [`update_form`](#update-form) [`delete_form`](#delete-form)                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `owner_client_id`<sup>**2**</sup>        | Yes      | `string`   | Required only when authorizing via [PATs](/authorization/personal-access-tokens). When you provide this param while authorizing with a Bearer Token, the `client_id` associated with the Bearer Token will be ignored, and the provided `owner_client_id` will be used instead. |
...
Bans a customer for a specified duration. While banned, the customer cannot connect to or interact with the [Customer Chat API](/messaging/customer-chat-api) within your organization.

#### Specifics

|                                  |                                                                      |
| -------------------------------- | -------------------------------------------------------------------- |
| Method URL                       | `https://api.livechatinc.com/v3.7/configuration/action/ban_customer` |
...
| Parameter     | Data type | Required | Notes                                                       |
| ------------- | --------- | -------- | ----------------------------------------------------------- |
| `seconds`     | `int`     | Yes      | The ban duration in seconds. Must be a positive integer.    |
| `ip`          | `string`  | Yes*     | The customer's IP address.                                  |
| `customer_id` | `string`  | Yes*     | The customer ID.                                            |

**1)** At least one of `ip` and `customer_id` is required. If `ip` is provided, it cannot match the license's `monitoring.ip_mask` property.
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.7/configuration/action/ban_customer \
  -H 'Authorization: Basic <your_personal_access_token>' \
  -H 'Content-Type: application/json' \
  -d '{
        "ip": "192.168.1.1",
...
<CodeResponse type="configuration" version="v3.7" json="banCustomer"/>
...
| Parameter         | Required | Data type | Notes                                                                                                                                                                                                                                                                           |
| ----------------- | -------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `owner_client_id` | Yes      | `string`  | Required only when authorizing via [PATs](/authorization/personal-access-tokens). When you provide this param while authorizing with a Bearer Token, the `client_id` associated with the Bearer Token will be ignored, and the provided `owner_client_id` will be used instead. |
...
| Required scopes                  | `canned_responses--groups:rw` `canned_responses--all:rw`<sup>**1**</sup>       |
| [Batch support](#batch-requests) | No                                                                             |

**1)** Required to update canned responses in any group
...
| Required scopes                  | `canned_responses--groups:ro`<sup>**1**</sup> `canned_responses--all:ro`<sup>**2**</sup>                                                |
| [Batch support](#batch-requests) | No                                                                                                                                      |

**1)** Required for accessing canned responses in groups the agent belongs to<br/>
**2)** Required when using the `include_all_private` parameter
...
| `include_all_private` | No       | `bool`          | If set to `true`: all private canned responses will be included. Requires `canned_responses--all:ro` scope. Default: `false`. |
...
**1)** Required to delete canned responses in any group
...
Forms let you read and manage forms configured for a group. Two families of forms are supported:

- **Survey forms** – the pre-chat and post-chat surveys (`prechat`, `postchat`).
- **Custom forms** – the other chat-widget forms (`offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm`).

The required scope depends on the form family, which is determined by the `type` passed in the payload:

| Form family  | `type` values                                                     | Required scopes                   |
| ------------ | ----------------------------------------------------------------- | --------------------------------- |
| Survey forms | `prechat`, `postchat`                                             | `surveys_read` / `surveys_manage` |
| Custom forms | `offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm` | `forms_manage`                    |

<Warning>

For custom forms, the `forms_manage` scope is required for both reading (`get_form`) and writing (`update_form`, `delete_form`) — there is no separate read scope.
...
| `type`    | `string`       | The form type. One of: `prechat`, `postchat`, `offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm`. |
| `enabled` | `bool`         | `true` when the form is enabled. Applies to survey forms only; custom forms don't use it. |
| `fields`  | `array[Field]` | An array of the form fields, in display order.                                 |

##### Field

| Parameter              | Data type       | Notes                                                                                        |
| ---------------------- | --------------- | -------------------------------------------------------------------------------------------- |
| `id`                   | `string`        | The field ID. Assigned by the server; ignored when updating.                                 |
| `type`                 | `string`        | The field type. The allowed values depend on the form `type` (see below).                    |
| `label`                | `string`        | The field label. Required. Must not contain tab characters.                                  |
| `required`             | `bool`          | `true` when the field is required.                                                           |
| `answers`              | `array[Answer]` | The available answers. Required for `radio`, `select`, `checkbox` and `group` fields.        |
| `comment_label`        | `string`        | The comment label. Required for `rating` fields. Survey forms only.                          |
| `confirmation_text`    | `string`        | The confirmation text. Required for `email_with_confirmation` fields. Survey forms only.     |
| `confirmation_checked` | `bool`          | Whether the confirmation is checked by default. Applies to `email_with_confirmation` fields. Survey forms only. |

###### Answer

| Parameter           | Data type | Notes                                                                                                                                                       |
| ------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `label`             | `string`  | The answer label. Required. Must not contain tab characters.                                                                                                |
| `checked`           | `bool`    | Whether the answer is checked by default. Survey forms only: required for `radio`, `select` and `checkbox` fields; for `radio` and `select` at most one answer may be checked. |
| `destination_group` | `number`  | The group the customer is routed to when the answer is selected. Required for `group` field answers. Survey forms only.                                     |

For custom forms, answers use only the `label` field; `checked` and `destination_group` are ignored.

Allowed field types per form type:

| Form type          | Allowed field types                                                                             |
| ------------------ | ----------------------------------------------------------------------------------------------- |
| `prechat`          | `header`, `name`, `email`, `email_with_confirmation`, `question`, `radio`, `select`, `checkbox`, `group` |
| `postchat`         | `header`, `question`, `radio`, `select`, `checkbox`, `textarea`, `rating`                       |
| `offline`          | `header`, `name`, `question`, `email`, `radio`, `select`, `checkbox`, `textarea`, `subject`     |
| `ask_for_email`    | `header`, `name`, `question`, `email`, `radio`, `select`, `checkbox`, `textarea`, `subject`     |
| `ask_for_email_mm` | `header`, `name`, `question`, `email`, `radio`, `select`, `checkbox`, `textarea`, `subject`     |
| `queued`           | `header`                                                                                        |
| `email`            | `email`                                                                                         |

A form can contain up to 20 fields, and each `radio`, `select`, `checkbox` or `group` field up to 350 answers. Only one `group` field and one `email_with_confirmation` field are allowed per form (survey forms). The `ask_for_email` form must contain an `email` field.

### Get Form

Returns a single form together with its fields.

#### Specifics

|                                  |                                                                  |
| -------------------------------- | ---------------------------------------------------------------- |
| Method URL                       | `https://api.livechatinc.com/v3.7/configuration/action/get_form` |
| Required scopes                  | `surveys_read`<sup>**1**</sup> `forms_manage`<sup>**1**</sup>    |
| [Batch support](#batch-requests) | No                                                               |

**1)** The scope depends on the form `type`. Survey forms (`prechat`, `postchat`) require `surveys_read`; custom forms (`offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm`) require `forms_manage`.

#### Request

| Parameter  | Required | Data type | Notes                                                |
| ---------- | -------- | --------- | ---------------------------------------------------- |
| `type`     | Yes      | `string`  | The form type. One of: `prechat`, `postchat`, `offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm`. |
| `group_id` | No       | `number`  | The ID of the group the form belongs to. Default: 0. |

#### Response

| Parameter | Data type | Notes               |
| --------- | --------- | ------------------- |
| `form`    | `Form`    | The requested form. |
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.7/configuration/action/get_form \
  -H 'Authorization: Basic <your_personal_access_token>' \
  -H 'Content-Type: application/json' \
  -d '{
    "type": "prechat",
...
Creates or updates a form. The whole form is replaced with the provided configuration, so include all the fields that should remain. Returns the updated form.

#### Specifics

|                                  |                                                                     |
| -------------------------------- | ------------------------------------------------------------------- |
| Method URL                       | `https://api.livechatinc.com/v3.7/configuration/action/update_form` |
| Required scopes                  | `surveys_manage`<sup>**1**</sup> `forms_manage`<sup>**1**</sup>     |
| [Batch support](#batch-requests) | No                                                                  |

**1)** The scope depends on the form `type`. Survey forms (`prechat`, `postchat`) require `surveys_manage`; custom forms (`offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm`) require `forms_manage`.
...
| `type`     | Yes      | `string`       | The form type. One of: `prechat`, `postchat`, `offline`, `queued`, `email`, `ask_for_email`, `ask_for_email_mm`. |
| `group_id` | No       | `number`       | The ID of the group the form belongs to. Default: 0.                                |
| `enabled`  | No       | `bool`         | If set to `true`: the form will be enabled. If set to `false`: it will be disabled. Survey forms only; ignored for custom forms. |
| `fields`   | See notes | `array[Field]` | The form fields. For survey forms it's optional — when omitted, the existing fields are left unchanged. For custom forms it's required and must contain at least one entry. |
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.7/configuration/action/update_form \
  -H 'Authorization: Basic <your_personal_access_token>' \
  -H 'Content-Type: application/json' \
  -d '{
    "type": "prechat",
...
The example below updates a custom form (`ask_for_email`). Custom forms require the `forms_manage` scope, `fields` must contain at least one entry, and answers use only the `label` field.
...
Deletes an existing form. For survey forms (`prechat`, `postchat`) the form is deactivated; for custom forms the form configuration is removed.

#### Specifics

|                                  |                                                                     |
| -------------------------------- | ------------------------------------------------------------------- |
| Method URL                       | `https://api.livechatinc.com/v3.7/configuration/action/delete_form` |
| Required scopes                  | `surveys_manage`<sup>**1**</sup> `forms_manage`<sup>**1**</sup>     |
| [Batch support](#batch-requests) | No                                                                  |
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.7/configuration/action/delete_form \
  -H 'Authorization: Basic <your_personal_access_token>' \
  -H 'Content-Type: application/json' \
  -d '{
    "type": "prechat",
