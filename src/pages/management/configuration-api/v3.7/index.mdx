...
Forms let you read and manage forms configured for a group.

<Warning>

Currently only the survey forms are supported, so the `type` parameter accepts `prechat` and `postchat`. More form types will be added in the future, and they will require their own scopes.

</Warning>

The required scope depends on the form `type` passed in the payload. For the survey forms (`prechat` and `postchat`) the methods require the `surveys_read` / `surveys_manage` scopes.
...
| `type`    | `string`       | The form type. One of: `prechat`, `postchat`.  |
| `enabled` | `bool`         | `true` when the form is enabled.               |
...
| `comment_label`        | `string`        | The comment label. Required for `rating` fields.                                             |
| `confirmation_text`    | `string`        | The confirmation text. Required for `email_with_confirmation` fields.                        |
| `confirmation_checked` | `bool`          | Whether the confirmation is checked by default. Applies to `email_with_confirmation` fields. |
...
| `checked`           | `bool`    | Whether the answer is checked by default. Required for `radio`, `select` and `checkbox` fields. For `radio` and `select` at most one answer may be checked. |
| `destination_group` | `number`  | The group the customer is routed to when the answer is selected. Required for `group` field answers.                                                        |

Allowed field types per form type:

| Form type  | Allowed field types                                                                                           |
| ---------- | ------------------------------------------------------------------------------------------------------------- |
| `prechat`  | `name`, `email`, `email_with_confirmation`, `question`, `radio`, `select`, `checkbox`, `group`, `information` |
| `postchat` | `header`, `question`, `radio`, `select`, `checkbox`, `textarea`, `rating`                                     |

A form can contain up to 20 fields, and each `radio`, `select`, `checkbox` or `group` field up to 350 answers. Only one `group` field and one `email_with_confirmation` field are allowed per form.
...
| Required scopes                  | `surveys_read`<sup>**1**</sup>                                   |
| [Batch support](#batch-requests) | No                                                               |

**1)** The scope depends on the form `type`. The survey forms (`prechat`, `postchat`) require `surveys_read`.

#### Request

| Parameter  | Required | Data type | Notes                                                |
| ---------- | -------- | --------- | ---------------------------------------------------- |
| `type`     | Yes      | `string`  | The form type. One of: `prechat`, `postchat`.        |
...
| Required scopes                  | `surveys_manage`<sup>**1**</sup>                                    |
| [Batch support](#batch-requests) | No                                                                  |

**1)** The scope depends on the form `type`. The survey forms (`prechat`, `postchat`) require `surveys_manage`.
...
| `type`     | Yes      | `string`       | The form type. One of: `prechat`, `postchat`.                                       |
| `group_id` | No       | `number`       | The ID of the group the form belongs to. Default: 0.                                |
| `enabled`  | No       | `bool`         | If set to `true`: the form will be enabled. If set to `false`: it will be disabled. |
| `fields`   | No       | `array[Field]` | The form fields. When omitted, the existing fields are left unchanged.              |
...
Deletes (deactivates) an existing form.
