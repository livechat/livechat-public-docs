...
| ---------------------- | --------- | -------------------------------------------------------------- |
| `domains`              | `array`   | Contains `domain` objects.                                     |
| `domains[].domain`     | `string`  | The domain name.                                               |
| `domains[].created_at` | `string`  | Date & time of adding the domain, in ISO 8601 format.          |
| `domains[].created_by` | `string`  | The email of the agent who added the domain.                   |
| `domains[].updated_at` | `string`  | Date & time of the last domain update, in ISO 8601 format.     |
| `domains[].updated_by` | `string`  | The email of the agent who last updated the domain.            |
...
```json
{
  "domains": [
    {
      "domain": "example.com",
      "created_at": "2024-03-11T10:15:32.000000Z",
      "created_by": "john.doe@example.com",
      "updated_at": "2024-05-02T14:27:11.000000Z",
      "updated_by": "john.doe@example.com"
    },
    {
      "domain": "subdomain.example.com",
      "created_at": "2024-04-18T08:03:45.000000Z",
      "created_by": "jane.doe@example.com",
      "updated_at": "2024-04-18T08:03:45.000000Z",
...
| `fields`  | `array[Field]` | An array of the form [fields](#####field) in display order.                                 |

##### Field

A form can contain up to 20 fields. Each radio, select, checkbox, or group field can contain up to 350 answers.

| Parameter              | Data type       | Notes                                                                                        |
| ---------------------- | --------------- | -------------------------------------------------------------------------------------------- |
| `id`                   | `string`        | The field ID. Assigned by the server and ignored when updating.                                 |
| `type`                 | `string`        | The [field type](######field-types). The allowed values depend on the form `type`.                     |
| `label`                | `string`        | The field label. Required. Must not contain tab characters.                                  |
| `required`             | `bool`          | `true` when the field is required.                                                           |
| `answers`              | `array[Answer]` | The available [answers](######answer). Required for `radio`, `select`, `checkbox`, and `group` fields.        |
...
Only one `group` field and one `email_with_confirmation` field are allowed per survey form. The `ask_for_email` form type must contain an `email` field.

| Form type          | Allowed field types                                                                             |
| ------------------ | ----------------------------------------------------------------------------------------------- |
| `prechat`          | `header`, `name`, `email`, `email_with_confirmation`, `question`, `radio`, `select`, `checkbox`, `group`, `phone`, `date` |
...
| Parameter           | Data type | Notes                                                                                                                                                       |
| ------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `label`             | `string`  | The answer label. Required. Can't contain tab characters.                                                                                                |
| `checked`           | `bool`    | Whether the answer is checked by default. Survey forms only: required for `radio`, `select`, and `checkbox` fields; for `radio` and `select`, only one answer may be checked. |
...
| Required scopes                  | `surveys_read`<sup>**1**</sup> or `forms_manage`<sup>**1**</sup> |
| [Batch support](#batch-requests) | No                                                               |
...
| Parameter | Data type | Notes               |
| --------- | --------- | ------------------- |
| `form`    | `Form`    | The requested [form](####form). |
...
| Required scopes                  | `surveys_manage`<sup>**1**</sup> or `forms_manage`<sup>**1**</sup>  |
| [Batch support](#batch-requests) | No                                                                  |
...
| `fields`   | See notes | `array[Field]` | The form [fields](#####field). For survey forms, it's optional — when omitted, the existing fields are left unchanged. For custom forms, it's required and must contain at least one entry. |

#### Response

| Parameter | Data type | Notes             |
| --------- | --------- | ----------------- |
| `form`    | `Form`    | The updated [form](####form). |
