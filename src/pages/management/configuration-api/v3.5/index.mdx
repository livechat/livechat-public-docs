...
It is possible to execute a method multiple times with different payloads using a single request, thanks to batch requests. Each request in a batch is executed concurrently, there are no guarantees in regards to execution order. The response for each method is returned in an array, in the same order as in the request. There is no possibility to use different methods in a single batch. Batch request requires the same scopes as it's non-bach equivalent.

## Request

| Parameter  | Required | Data type | Notes                                                      |
| ---------- | -------- | --------- | ---------------------------------------------------------- |
| `requests` | yes      | `array`   | Array of Request objects of corresponding non-batch method |
...
| Resource Name | Notes                                                                                                                        |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `responses`   | Array of Response objects or [Errors](#errors) of corresponding non-batch method. The order is the same as the Request order |
