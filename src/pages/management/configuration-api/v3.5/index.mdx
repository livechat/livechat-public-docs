...
Tags are simple text labels that can be used by agents to assign chats to user-defined categories that can be used to filter chats out in archives and reports.

To create a new tag call [**Create Tag**](/management/configuration-api/v3.5/#create-tag) method with a `name`. Letter case is ignored when checking if the tag already exists. When creating a tag you can also provide `group_ids` to assign the tag to some groups right away. It's also possible to create a tag with empty `group_ids` and update the group assignment later with [**Update Tag**](/management/configuration-api/v3.5/#update-tag). To remove a tag call [**Delete Tag**](/management/configuration-api/v3.5/#delete-tag). Both Update and Delete methods respect the letter case when matching the tag in order to be backward-compatible with deprecated v2 endpoints. You can get existing tags with [**List Tags**](/management/configuration-api/v3.5/#list-tags) method.

Users with `tags--all:rw` scope can create/delete/update tags in all license's groups. Users with `tags--groups:rw` scope are restricted to groups they are assigned to. Similarly `tags--all:ro` and `tags--groups:ro` scopes affect listing tags.
...
| `group_ids` | yes      | `[]int`   | Can be empty.                                             |
