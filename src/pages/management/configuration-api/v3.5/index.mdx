...
### Get Product Limits for Plan

To keep services stable for all clients, LiveChat limits common resources. The limits are different for each pricing plan.
Method `check_products_limits_for_plan` shows what resources exceed its limits for any given plan. This information is useful before downgrading the plan.

Limited resources in LiveChat:

| Resource Name          | Notes                                                                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `agents`               | The max number of active agents __*__                                                                               |
| `groups`               | The max number of groups that can be created                                                                        |
| `banned`               | The max number of banned customers or IPs                                                                           |
| `queue`                | The max number of chats waiting in the queue ([Messaging Mode](/messaging#messaging-mode) is off)                   |
| `queue_continuous`     | The max number of chats waiting in the queue ([Messaging Mode](/messaging#messaging-mode) is on)                    |
| `groups_per_agent`     | The max number of groups an agent can be assigned to                                                                |
| `access_rules`         | The max number of [Access Rules](/management/configuration-api#auto-access)                                         |
| `group_chooser_groups` | The max number groups in [pre-chat survey](https://www.livechat.com/help/dividing-live-chat-by-group/#target-field) |

__*__ Including bots, excluding suspended or awaiting approval agents
