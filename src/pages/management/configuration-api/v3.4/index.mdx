...
If the requester has the `billing:rw` scope, then the newly created Agent won't have to wait for approval (`awaiting_approval: false`).
...
| Required scopes | `accounts--all:rc` `accounts--all:rw` (to create already approved agents) |
