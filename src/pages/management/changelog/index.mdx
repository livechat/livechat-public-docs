...
- The [**Create Agent**](/management/configuration-api/v3.6/#create-agent) property `avatar` is removed. Use [**Update Account Organization Settings**](/authorization/global-accounts-api#tag/AccountOrganizationSettings/paths/~1accounts~1%7Baccount_id%7D~1organization_settings~1%7Borganization_id%7D/patch) to update `avatar`.
- The [**Update Agent**](/management/configuration-api/v3.6/#update-agent) property `name` and `avatar` are now removed.
