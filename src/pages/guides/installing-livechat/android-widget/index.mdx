...
Network calls require at least `INFO`. `DEBUG` and `VERBOSE` provide maximum level of detail
...
> **Note:** Using `ACTIVITY` mode will disable the `NewMessageListener` no longer works when chat is not visible.
