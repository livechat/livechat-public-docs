...
The LiveChat Partner API provides a RESTful interface that enables partners to programmatically access and manage their partnership data. This includes functionalities such as retrieving earnings reports, managing customer licenses, and overseeing affiliate campaigns.
...
Partner authentication is handled via API tokens, which can be generated within the Partner App. These tokens must be included in the Authorization header of your HTTP requests to authenticate and authorize access to the API endpoints.
...
- [Get Commission Metric](#get-commission-metric)
...
`GET https://api.livechatinc.com/v2/partners` - get the profile info
...
`PUT https://api.livechatinc.com/v2/partners/profile` - update the profile info
...
`GET https://api.livechatinc.com/v2/partners/stats` - get the number of licenses for each product and partnership model
...
`GET https://api.livechatinc.com/v2/partners/coupons` - get the list of active coupons
...
`GET https://api.livechatinc.com/v2/partners/earnings` - get earnings (commission earned, balance, blocked and pending amount, and more)
...
`GET https://api.livechatinc.com/v2/partners/reports/commission` - get commission reports
...
- `filters` is an array of objects, where each object has the following fields:
  - `field` - field name (one of: `program`, `product`)
  - `value` - field value
  - `operator` - operator (`equals`)
- for example: `filters=[{"field": "prod", "value": "livechat", "operator": "equals"}]`
...
`GET https://api.livechatinc.com/v2/partners/partners-referred/reports/commission` - get commission earned from referring partners
...
`GET https://api.livechatinc.com/v2/partners/billing` - get billing information
...
`GET https://api.livechatinc.com/v2/partners/balance` - get balance information
...
`GET https://api.livechatinc.com/v2/partners/metrics/commission` - get commission metric
...
- `date_from` - get commission metric from a given date
- `date_to` - get commission metric up to a given date
- `product` - get commission metric for a given product (one of: `livechat`, `chatbot`, `helpdesk`, `all`; default: `all`)
- `program` - get commission metric for a given program (one of: `solution`, `affiliate`, `all`; default: `all`)
...
`PUT https://api.livechatinc.com/v2/partners/billing` - update billing information
...
`POST https://api.livechatinc.com/v2/partners/withdrawals` - create a withdrawal request
...
`GET https://api.livechatinc.com/v2/partners/withdrawals` - get the list of withdrawals
...
`POST https://api.livechatinc.com/v2/partners/tokens` - create a new API token
...
`GET https://api.livechatinc.com/v2/partners/tokens` - get the list of active API tokens
...
`DELETE https://api.livechatinc.com/v2/partners/tokens/<id>` - a revoke API token
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/<status>` - get solution licenses of a certain status
...
- [Get Campaigns Earnings Metric](#get-campaigns-earnings-metric)
- [Get Campaigns Earnings Report](#get-campaigns-earnings-report)
- [Get Campaigns Summary](#get-campaigns-summary)
...
`POST https://api.livechatinc.com/v2/partners/affiliates/campaigns` - create a new campaign
...
- `product` - **required** - product (`livechat`, `chatbot`, `helpdesk`, or `partner_program` itself)

#### Body

- `name` - **required** - campaign name (min. 5, max. 100 characters)
- `link` - **required** the URL of the product landing page (default: product homepage)
- `trial_duration` - **required** - the number of days of a trial period (min. 14 - default, max. 30)
- `coupon_id` - the ID of the coupon assigned to a Partner (a discount will be applied)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/search` - search for campaigns
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns` - get campaigns
...
`DELETE https://api.livechatinc.com/v2/partners/affiliates/campaigns/<campaign_id>` - disable a campaign
...
`GET https://api.livechatinc.com/v2/partners/affiliates/licenses` - get the list of Affiliate licenses
...
`GET https://api.livechatinc.com/v2/partners/affiliates/performance` - get Affiliate's performance report
...
`GET https://api.livechatinc.com/v2/partners/affiliates/partners-referred` - get the list of referred Partners
...
`GET https://api.livechatinc.com/v2/partners/affiliates/links` - get affiliate links (long and short version) for default campaigns of each product
...
`GET https://api.livechatinc.com/v2/partners/affiliates/metrics/trials` - get number of trials generated from Affiliate campaigns
...
- `date_from` - **required** - get trials metrics from a given date
- `date_to` - **required** - get trials metrics up to a given date
...
`GET https://api.livechatinc.com/v2/partners/affiliates/metrics/earnings` - get earnings generated from Affiliate campaigns
...
- `date_from` - **required** - get earnings metrics from a given date
- `date_to` - **required** - get earnings metrics up to a given date
- `product` - optional - get earnings metrics for a given product (one of: `livechat`, `chatbot`, `helpdesk`)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/metrics/conversions` - get number of trials converted into paid licenses generated from Affiliate campaigns
...
- `date_from` - **required** - get conversions metrics from a given date
- `date_to` - **required** - get conversions metrics up to a given date
...
`GET https://api.livechatinc.com/v2/partners/affiliates/metrics/clicks` - get clicks generated from Affiliate campaigns
...
- `date_from` - **required** - get clicks metrics from a given date
- `date_to` - **required** - get clicks metrics up to a given date
...
`GET https://api.livechatinc.com/v2/partners/partners-referred/metrics/commission` - get commission generated from referred partners activity
...
- `product` - get commission metric for a given product (one of: `livechat`, `chatbot`, `helpdesk`, `partner_program`, `all`; default: `all`)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/reports/trials` - get trials report for the Affiliate activity over time (up to last 12 months)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/reports/earnings` - get earnings report for the Affiliate activity over time (up to last 12 months)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/reports/conversions` - get conversions report for the Affiliate activity over time (up to last 12 months)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/reports/clicks` - get clicks report for the Affiliate activity over time (up to last 12 months)
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/<campaign_id>/metrics/earnings` - get affiliate campaign earnings metric
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/<campaign_id>/reports/earnings` - get affiliate campaign earnings report
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/<campaign_id>/summary` - get affiliate campaigns summary
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/reports/clicks` - get clicks report for a given number of Best Performing campaigns
...
- `limit` - **required** - number of campaigns to return
- `date_from` - **required** - get clicks report from a given date
- `date_to` - **required** - get clicks report up to a given date
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/reports/trials` - get trials report for a given number of Best Performing campaigns
...
- `limit` - **required** - number of campaigns to return
- `date_from` - **required** - get trials report from a given date
- `date_to` - **required** - get trials report up to a given date
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/reports/conversions` - get conversions report for a given number of Best Performing campaigns
...
- `limit` - **required** - number of campaigns to return
- `date_from` - **required** - get conversions report from a given date
- `date_to` - **required** - get conversions report up to a given date
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/reports/earnings` - get earnings report for a given number of Best Performing campaigns
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/metrics/clicks` - get summarized number of clicks generated from a given number of Best Performing campaigns
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/metrics/trials` - get summarized number of trials generated from a given number of Best Performing campaigns
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/metrics/conversions` - get summarized number of trials converted into paid licenses generated from a given number of Best Performing campaigns
...
`GET https://api.livechatinc.com/v2/partners/affiliates/campaigns/best-performing/metrics/earnings` - get summarized earnings generated from a given number of Best Performing campaigns
...
`GET https://api.livechatinc.com/v2/partners/affiliates/summary` - get a number of trial affiliate licenses, paid affiliate licenses, and expired affiliate licenses
...
- [Get Solution Licenses CSV Report](#get-solution-licenses-csv-report)
...
- [Get Lead History](#get-lead-history)
- [Submit Lead](#submit-lead)
- [Get Solution Summary](#get-solution-summary)
...
`GET https://api.livechatinc.com/v2/partners/solutions/organizations/search` - search managed organizations
...
- `filters` - URL-encoded JSON array of filter objects. Each filter has the format: `{"field": "", "operator": "", "value": ""}`

**Available filter fields:**

- `organization_id` - filter by organization ID (operators: `equals`, `contains`)
- `organization_name` - filter by organization name (operators: `equals`, `contains`)
- `client_name` - filter by client name (operators: `equals`, `contains`)
- `client_email` - filter by client email (operators: `equals`, `contains`)
- `identifier` - filter by custom identifier (operators: `equals`, `contains`)
- `license_id` - filter by license ID (operators: `equals`, `contains`)
- `products` - filter by product (operators: `one_of`; values: `livechat`, `chatbot`, `helpdesk`)
- `arr` - filter by annual recurring revenue as decimal string (operators: `greater_or_equal`, `lesser_or_equal`; example value: `"0.1"`)

#### Response

- `200 - Successful operation` - success, see example
- `403 - Access forbidden` - access forbidden
...
```shell
curl --request GET \
--url 'https://api.livechatinc.com/v2/partners/solutions/organizations/search?page=1&limit=15&filters=%5B%7B%22field%22%3A%22organization_id%22%2C%22operator%22%3A%22contains%22%2C%22value%22%3A%22abc%22%7D%5D' \
...
`GET https://api.livechatinc.com/v2/partners/solutions/organizations/<organization_id>` - get organization details from Account API
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses` - get the list of licenses
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/<status>` - get the list of licenses by status
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>` - get the license details
...
`PUT https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>` - update the license details
...
`POST https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>/operators` - invite operators to a license
...
- `<product>` - **required** - product (`livechat` or `helpdesk`)
- `<license_id>` - **required** - license ID
...
- `role` - **required** - `administrator` or `agent` (applicable for all products), and `viever` (only for the `helpdesk` product)
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/<license_id>/invoices` - get invoices for a given license
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/<license_id>/invoices/<invoice_no>` - get a PDF invoice
...
`POST https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>/subscription` - create a new subscription for a license
...
- `billing_cycle` - **required** - billing cycle, `monthly` or `annual`
- `seats` - **required** - the number of users
...
`PUT https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>/subscription` - update a subscription for a license
...
`DELETE https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>/subscription` - close a subscription for a license
...
`PUT https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>/billing` - update the subscription billing information for a license
...
`POST https://api.livechatinc.com/v2/partners/solutions/licenses/<product>/<license_id>/transfer` - transfer a license to the client
...
- `new_owner` - **required** - the new owner's email (one of existing operators)
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/count` - get number of trials, paid licenses, and expired licenses from the Solution program.
...
`GET https://api.livechatinc.com/v2/partners/solutions/reports/license` - get report for licenses by license status.
...
- `date_to` - **required** - get report up to a given date
- `aggregation` - **required** - get report aggregated by a given timespan (one of: `day`, `week`, `month`)
- `license_status` - **required** - get report for a given licenses status (one of: `trial`, `paid`, `expired`)
...
`GET https://api.livechatinc.com/v2/partners/solutions/licenses/search/csv` - get CSV report for licenses.
...
**Note:**
- `filters` (URL-encoded) is an array of objects, where each object has the following fields:
...
`GET https://api.livechatinc.com/v2/partners/solutions/leads` - get partner leads
...
`GET https://api.livechatinc.com/v2/partners/solutions/leads/{leadID}` - get lead details
...
`GET https://api.livechatinc.com/v2/partners/solutions/leads/{leadID}/history` - get lead history
...
`POST https://api.livechatinc.com/v2/partners/solutions/leads` - submit lead
...
- `first_name` - **required** - first name
- `last_name` - **required** - last name
- `region` - lead region (country code)
- `job_title` - job title
...
- `monetization_type` -  **required** - `flat_fee` or `rev_share`, [check the difference here](https://partners.livechat.com/help/lead-submission-your-newest-opportunity/#what-are-the-criteria-for-lead-submission)
...
`GET https://api.livechatinc.com/v2/partners/solutions/summary` - get summary from Solution Program
...
`POST https://api.livechatinc.com/v2/partners/experts/profile/logo` - upload the Expert logo
...
`POST https://api.livechatinc.com/v2/partners/experts/profile` - create the Expert profile
...
- `categories` - **required** - an array of categories (codes)<sup>1</sup>
- `industry_ids` - **required** - industry specializations<sup>2</sup>
- `projects` - **required** - an array of projects (URLs)
- `agents` - **required** if one of the categories is `outsourcing-customer-service` - number of agents, accepted values: `1-5`, `6-15`, `16-30`, `31-50`, `51+`
- `languages` - **required** if one of the categories is `outsourcing-customer-service` - an array of languages (codes); [available languages](https://api.livechatinc.com/v2/partners/data/languages)
- `hour_rate` - minimum hourly rate
- `project_rate` - minimum project rate
- `facebook` - the URL to the Facebook page
- `twitter` - the URL to the Twitter page
- `linkedin` - the URL to the LinkedIn page
...
`GET https://api.livechatinc.com/v2/partners/experts/profile` - get the Expert profile details
...
`PUT https://api.livechatinc.com/v2/partners/experts/profile` - update the Expert profile details
...
`GET https://api.livechatinc.com/v2/partners/experts/comments` - get comments on the Expert profile
...
`GET https://api.livechatinc.com/v2/partners/experts/stats` - get stats ([Experts Marketplace](https://www.livechat.com/marketplace/experts/) position, number of comments and messages) for Expert profile
...
`GET https://api.livechatinc.com/v2/partners/experts/history` - get historical stats for the Expert profile views (up to last 12 months)
