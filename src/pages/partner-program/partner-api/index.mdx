...
Returns solution licenses of a certain status.
...
| Parameter     | Required | Data type | Notes                                                                    |
| ------------- | -------- | --------- | ------------------------------------------------------------------------ |
| `<status>`    | Yes      | `string`  | The license status. Possible values: `trial`, `paid`, `expired`.         |
| `date_from`   | Yes      | `string`  | The date to get licenses from. Date format: `YYYY-MM-DDTHH:mm:ssZ`.      |
| `date_to`     | Yes      | `string`  | The date to get licenses up to. Date format: `YYYY-MM-DDTHH:mm:ssZ`.     |
| `limit`       | No       | `int`     | A certain amount of licenses in the response. Min. 1, max. 15 (default). |
| `next_cursor` | No       | `string`  | The cursor for fetching the next batch of licenses.                      |
...
```shell
curl --request PUT \
  --url https://api.livechatinc.com/v2/solutions/licenses/livechat/104130623 \
  --header 'Authorization: Bearer <YOUR_API_TOKEN>' \
...
```shell
curl --request POST \
  --url 'https://api.labs.livechatinc.com/v2/partners/solutions/licenses/livechat/104130623/operators' \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer <YOUR_API_TOKEN>' \
  --data '{
    "operators": [
