...
To do that, we'll send an HTTP POST request providing `code`, **Client Id**, **Client Secret**, and **Redirect URI**. To do that, we'll use Postman. Go to <a href="https://developers.livechat.com/docs/authorization/agent-authorization#step-4-exchange-code-for-access-token-and-refresh-token" target="_blank">Exchange code for access token and refresh token</a> in our documentation and copy the request code snippet. Please replace sample parameters with your own.
