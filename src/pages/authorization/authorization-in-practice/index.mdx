...
Go to <a href="https://platform.text.com/docs/messaging/agent-chat-api/v3.5/#list-chats" target="_blank">List Chats</a> in our documentation and copy the request code snippet. Once more, we'll use Postman and its **Import > Paste Raw Text** option. After importing, replace ```<your_access_token>``` with the actual token.
...
In case of doubts, make sure to check out our [documentation](https://platform.text.com/docs/messaging/). You can always reach out to us at [developers@text.com](mailto:developers@text.com).
