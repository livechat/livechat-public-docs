...
1. Run a `txdev` command. <br/><br/>Running a command will automatically update your app in the Developer Console. After that, pull the changes to the app manifest (`txdev app pull`) as it doesn't update automatically. <br/><br/>We recommend this method to apply **more complex changes**, such as adding an enitre building block.
...
**1. Install the Developer CLI globally.**

<CodeSample path={'1. Install Developer CLI'}>
...
**2. Log in to Text Developer Console using the CLI.**

<CodeSample path={'2. Log in to Developer Console'}>
...
**3. Clone an existing app to your local environment.**

The CLI will prompt you to select an app.

<CodeSample path={'3. Clone an app'}>
...
**5. Update the local app manifest.**

After executing a `txdex` command, you need to pull the changes to your local app manifest (`txdev app pull`) as it doesn't update automatically.

<CodeSample path={'5. Pull app manifest changes'}>
...
Now you can go to the <Placeholder id="DEVELOPER_CONSOLE_URL" /> to see the changes you've made.
