...
- by executing a `txdev` command. Running a command will automatically update your app in the Developer Console.
- by manipulating the JSON file manually. Run `txdev app push` to push the changes to the Developer Console.

💡 `txdex app push/pull/verify` are to be used to manage only the app manifest. To download or update other app files (e.g. env variables), run `txdev app clone`.
