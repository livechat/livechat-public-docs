...
After your user signs out of the app, you might want to clear the chat session. You can do that by invoking a static method on `ChatWindowUtils.clearSession(Context)` from anywhere in the app. In case your `ChatWindowView` is attached in course of the log out flow, you also going to need to reload it by calling `chatWindow.reload(false)` after the `clearSession` code. See <a href="https://github.com/livechat/chat-window-android/blob/master/app/src/main/java/com/livechatinc/livechatwidgetexample/FullScreenWindowActivityExample.java" target="_blank" rel="noopener noreferrer">FullScreenWindowActivityExample.java</a>.
