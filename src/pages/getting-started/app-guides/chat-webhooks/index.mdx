...
The document applies to the so called **new chat webhooks** (v3.3). It's impossible to configure the old chat webhooks (v3.2 or below) via Developer Console – you can only delete the already configured ones or [migrate them](#webhooks-migration) to the current stable version. If you're curious about the differences between the old and new chat webhooks, [read this article](https://developers.livechat.com/updates/setting-up-webhooks-with-livechat-api).
