...
To finish off, install the app privately (**Private installation** tab in Developer Console) – it will trigger the Direct installation flow for your app. Then, perform the action that's your webhook trigger (for example, start a chat if you selected the `incoming_chat` webhook), and  As a result, your Webhook URl should receive the registered webhook.
