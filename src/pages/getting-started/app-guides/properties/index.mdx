...
The Developer Console makes it possible to creating properties via user interface instead of calling the [Register Property](/management/configuration-api/#register-property) method directly. LiveChat properties are registered per application (Client ID). An application that owns a property (property configuration/definition, not a specific property value) is the owner Client ID.
...
💡 Keep in mind that property publication is irreversible. Also, once published, a property cannot be edited or deleted.
