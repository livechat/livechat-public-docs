...
Indicates about an incoming [rich message](/messaging/agent-chat-api/v3.7/data-structures/#rich-message) postback. The push payload contains the info on the postback itself, as well as the chat it was sent in.
...
Indicates about customers the agent should be aware of. It's sent after [`login`](/messaging/agent-chat-api/v3.7/rtm-reference#login).
...
### `customer_properties_values_updated`
...
- for the added [auto access](/management/configuration-api/v3.7/#auto-access) rule, the payload contains the complete state of the auto access rule data structure, including empty fields.
- for the updated [auto access](/management/configuration-api/v3.7/#auto-access) rule, the payload contains only the updated properties.
- for the deleted [auto access](/management/configuration-api/v3.7/#auto-access) rule, the payload confirms the deletion.
...
Indicates that a canned response was created. The payload contains the full state of the canned response data structure.
...
Indicates that a canned response was updated. The payload contains the updated canned response data.
...
Indicates that a canned response was deleted.
...
### `customer_property_definition_created`

Indicates that a [customer property](/messaging/agent-chat-api/v3.7/data-structures#customer-property) definition was created within an organization. The payload contains the full state of the customer property definition.
...
### `customer_property_definition_updated`

Indicates that a [customer property](/messaging/agent-chat-api/v3.7/data-structures#customer-property) definition was updated within an organization. The payload contains the full current state of the customer property definition, including the fields that did not change.
...
Indicates that a [customer property](/messaging/agent-chat-api/v3.7/data-structures#customer-property) definition was deleted within an organization. The payload contains only the ID of the deleted definition. The property is no longer returned in the list of definitions, its values are no longer returned for any customer, and new values can't be set for it. The deletion is permanent.
...
Indicates an error occurred during processing of an asynchronous query (e.g. after calling [`request_thread_summary`](/messaging/agent-chat-api/v3.7/rtm-reference/#request-thread-summary)).
