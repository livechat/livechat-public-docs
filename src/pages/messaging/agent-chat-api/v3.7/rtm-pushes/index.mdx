...
Informs about an incoming [rich message](/messaging/agent-chat-api/v3.7/data-structures/#rich-message) postback. The push payload contains the info on the postback itself, as well as the chat it was sent in.
...
Informs about customers the agent should be aware of. It's sent after [`login`](/messaging/agent-chat-api/v3.7/rtm-reference#login).
...
Informs that the customer moved to another page of the website.
...
Informs that the customer was banned for a specified number of days.
...
Informs that the customer left the tracked website.
...
Informs about the message the customer is currently typing. The push payload contains the sneak peek object.
