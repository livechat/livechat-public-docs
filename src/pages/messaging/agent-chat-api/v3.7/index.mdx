...
**\*\*\*\*)** `agents-bot--all:rw` - to set the routing status of any bots within a license. Bots of type `ai_agent` are an exception. Their routing statuses can only be changed with tokens issued for the `client_id` that owns them.
