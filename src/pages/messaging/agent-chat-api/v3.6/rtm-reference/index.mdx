...
Sends an [Event](/messaging/agent-chat-api/v3.6/data-structures/#events) that isn't saved in the chat.
When preparing a longer response you can use this method to send a partial message before sending the final one with [Send Event](/messaging/agent-chat-api/v3.6/#send-event).
Only [Message](/messaging/agent-chat-api/v3.6/data-structures/#message) is supported. Visibility must be set to `all`, or skipped.
