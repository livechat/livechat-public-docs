...
<Section>
  <Text>
    ...
    The **DeletedEvent** event is returned to an agent who is not an admin. This event contains only a limited amount of
    data that is present in the original event.

    ### Request

    It is not possible to send a DeletedEvent in API version 3.6.
    ...
    | `id` | always | |
    | `created_at` | always | Date & time format with a resolution of microseconds, `UTC string`. |
    | `type` | always | Type of original event. |
    | `visibility` | always | |
    | `deleted` | always | |
  ...
  <Code>
    <CodeResponse version="v3.6" type="agent" title={'Sample DeletedEvent in response'} json="deletedEvent"/>
  </Code>
</Section>
