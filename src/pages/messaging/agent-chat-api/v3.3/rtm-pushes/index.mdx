...
Indicates about an incoming [event](#events) sent to a chat.
...
Indicates that an [event](#events) was updated.
...
Indicates about an incoming [rich message](/messaging/agent-chat-api/data-structures/#rich-message) postback. The push payload contains the info on the postback itself, as well as the chat it was sent in.
...
Indicates about customers the agent should be aware of. It's sent after [`login`](/messaging/agent-chat-api/v3.3/rtm-reference#login).
