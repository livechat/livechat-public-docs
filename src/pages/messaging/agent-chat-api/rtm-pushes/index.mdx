...
Indicates about an incoming [rich message](/messaging/agent-chat-api/v3.6/data-structures/#rich-message) postback. The push payload contains the info on the postback itself, as well as the chat it was sent in.
...
Indicates about customers the agent should be aware of. It's sent after [`login`](/messaging/agent-chat-api/v3.6/rtm-reference#login).
...
Indicates that the values of [customer properties](/messaging/agent-chat-api/data-structures#customer-property) were updated.
...
- for the added [auto access](/management/configuration-api/v3.6/#auto-access) rule, the payload contains the complete state of the auto access rule data structure, including empty fields.
- for the updated [auto access](/management/configuration-api/v3.6/#auto-access) rule, the payload contains only the updated properties.
- for the deleted [auto access](/management/configuration-api/v3.6/#auto-access) rule, the payload confirms the deletion.
...
Indicates that a [customer property](/messaging/agent-chat-api/data-structures#customer-property) definition was created within an organization. The payload contains the full state of the customer property definition.
...
Indicates that a [customer property](/messaging/agent-chat-api/data-structures#customer-property) definition was updated within an organization. The payload contains the full current state of the customer property definition, including the fields that did not change.
...
Indicates an error occurred during processing of an asynchronous query (e.g. after calling [`request_thread_summary`](/messaging/agent-chat-api/rtm-reference/#request-thread-summary)).
