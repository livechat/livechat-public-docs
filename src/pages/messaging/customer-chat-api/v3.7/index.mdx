...
| `continuous`             | No       | `bool`    | When set to `true`, starts chat as continuous (online group is not required). When set to `false`, starts chat as non-continuous. Default: `false`. |
...
| `phone_number`   | [Customer's](/messaging/customer-chat-api/v3.7/data-structures/#customer) phone number. Returned only if set. |
...
Gets the predicted agent - the one the customer will chat with when the chat starts. To use this method, the customer needs to be logged in, which can be done via the [`login`](/messaging/customer-chat-api/v3.7/rtm-reference/#login) method.

#### Specifics

|                        |                                                                                               |
| ---------------------- | --------------------------------------------------------------------------------------------- |
| **Method URL**         | `https://api.livechatinc.com/v3.7/customer/action/get_predicted_agent`                        |
| **RTM API equivalent** | [`get_predicted_agent`](/messaging/customer-chat-api/v3.7/rtm-reference/#get-predicted-agent) |
| **Webhook**            | -                                                                                             |
...
```shell
curl -X POST \
  'https://api.livechatinc.com/v3.7/customer/action/get_predicted_agent?organization_id=<organization_id>' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <customer_access_token>' \
  -d '{}'
```
