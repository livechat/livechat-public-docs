...
| Parameter | Required | Data type | Notes |
| --- | --- | --- | --- |
| `chat_id` | yes | `string` | Chat identifier. |
| `event_id` | yes | `string` | Rich message event identifier. |
| `postback` | yes | `object` | Postback payload describing the button action. |
| `postback.id` | yes | `string` | The postback name of the button. |
| `postback.type` | no | `string` | Action type associated with the clicked button, for example `add_to_cart`, `show_gallery`, or `open_external`. |
| `postback.ecommerce_option_id` | no | `string` | Selected variant option identifier related to the action. |
| `postback.toggled` | yes | `boolean` | The toggle state of the button. |
| `postback.content` | no | `object` | Additional key-value metadata sent with the postback. |
| `thread_id` | yes | `string` | Thread identifier. |
