...
Requests a welcome message from the system.
The response will contain a predicted agent, the one the customer will chat with when the chat starts.
The welcome message event will be generated asynchrounously and sent via the ([`incoming_welcome_message`](/messaging/customer-chat-api/v3.7/rtm-pushes/#incoming_welcome_message)) push.
To use this method, the customer needs to be logged in, which can be done via the [`login`](/messaging/customer-chat-api/v3.7/rtm-reference/#login) method.
