...
In the Agent Chat API, the [Delete Event](#delete-event) method replaces the original event with a [Deleted Event](/messaging/customer-chat-api/v3.6/data-structures#deleted-event) for agents with the `normal` role. Agents with higher roles (`administrator`, `viceowner`, and `owner`) see the original event with a `deleted` flag.
