...
Deletes an [Event](/messaging/customer-chat-api/v3.6/data-structures/#events) object. Deleted events are hidden from the Customer Chat API methods but remain visible via the Agent Chat API. The [Delete Event](#delete-event) method replaces the original event with a [Deleted Event](/messaging/customer-chat-api/v3.6/data-structures#deleted-event) to agents with the `normal` role. Agents with higher roles (`administrator`, `viceowner`, and `owner`) see the original event with a `deleted` flag.
