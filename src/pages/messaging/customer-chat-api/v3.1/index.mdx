...
This document describes the **Customer Chat Web API v3.1**. This version is **deprecated**. We encourage you to migrate to the latest stable version. [Read more about versioning...](/messaging/#api-versioning)
...
Changing the customer `name`, `email`, `avatar`, and `fields` while the customer has an active chat will succeed, but the change will apply after the chat has closed.
...
- When using RTM API, you can be also notified about events with [pushes](/messaging/customer-chat-api/v3.1/rtm-pushes).
