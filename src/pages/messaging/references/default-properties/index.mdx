...
Specifies routing priority for queued chats in the group.
...
Specifies the time in minutes after which the Agent inactivity form will be displayed. `-1` means that the timeout is disabled.
...
Specifies the time in minutes after last message after which a chat is archived.
...
Specifies the time in minutes after last message after which a continuous chat is archived.
...
Specifies the time in minutes after which a chat is transferred to another Agent if the previous one didn't respond. `0` means that the chat is not transferred.
...
Specifies a percentage threshold of queued chats after which the queue is reopened. This setting can be overridden per group by the [`queue.reopen_threshold.override`](#queue_reopen_thresholdoverride) property.
...
Specifies if the Agent who last chatted with the Customer should be preferred when routing a new chat.
...
Specifies the additional limit of concurrent active chats on the license.
...
Specifies the base limit of concurrent active chats on the license.
...
Specifies the hard limit of concurrent active chats on the license. The sum of additional and base limits cannot extend this value.
