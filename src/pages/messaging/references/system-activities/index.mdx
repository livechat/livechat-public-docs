...
Messaging source includes system events related to messaging activities, such as information about generation of chat summaries.

### summary

Generated as a result of generating thread summary in active threads. [Read more](/messaging/agent-chat-api#request-thread-summary).
...
| `summary.status` | Yes      | `string`  | The status of the summary generation process. Can be either `ok` or `error` |
| `requested_by`   | Yes      | `string`  | The user who requested the thread summary generation                        |

<Code>
    <CodeResponse title={'Sample system event payload'} type={'messaging-references'} json={'summaryV1'}/>
</Code>
