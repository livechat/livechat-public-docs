...
Shows how much the agent (or group or whole account) was available for chatting during specified period of time.
...
See [available parameters and filters](#available-parameters-and-filters). Depending on distribution parameter response will contain diffrent time resolution.

#### Response

| Field                    | Notes                                                                        |
| ------------------------ | -----------------------------------------------------------------------------|
| `total`                  | The total time agent was available in the specified date range.              |
| `records`                | Contains the `distribution` objects, for example, `day`.                     |
| `records.hours`          | Total hours agent was available that `day` (Only for `day` distribution)     |
| `records.minutes`        | Total minutes agent was available that `hour` (Only for `hour` distribution) |
...
```shell
curl -X POST \
https://api.livechatinc.com/v3.4/reports//availability \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <your_access_token>' \
  -d '{
    "filters": {
